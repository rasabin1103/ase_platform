export const catalogEn = {
  premium: {
    badge: 'Premium catalog',
    viewAll: 'View full catalog',
    productsTeaser: 'SaaS platforms curated for quality, governance and scale.',
    coursesTeaser: 'Expert-led paths with labs, templates and certificates.',
  },
  typeProduct: 'SaaS Product',
  typeCourse: 'Course',
  typeBook: 'Book',
  typeResource: 'Downloadable resource',
  groupLabels: {
    product: 'Products',
    course: 'Courses',
    book: 'Books',
    resource: 'Resources',
  },
  viewDetail: 'View detail',
  preview: 'Preview',
  addFavorite: 'Add to favorites',
  removeFavorite: 'Remove from favorites',
  buy: 'Buy',
  purchased: 'Purchased',
  free: 'Free',
  planSavings: {
    title: 'You could save on this',
    body: 'This item is included in a plan — subscribing may cost less than buying it (and everything else the plan includes) one by one.',
    planValue: 'Worth buying separately',
    planPrice: 'Plan price',
    savingsLabel: 'You save',
    perMonth: '/mo',
    itemsIncludedLabel: 'items included',
    buyAlone: 'Just buy this item',
    seePlan: 'See this plan',
  },
  series: {
    title: 'Series',
    progress: '{{owned}}/{{total}} owned',
    current: 'You are here',
    recommendedNext: 'Recommended next',
    complete: "You've got the whole series!",
  },
  loadError: 'Could not load the catalog. Try again later.',
  empty: 'No items in this catalog yet.',
  emptyHint: 'New products, courses, books and resources are added regularly — check back soon.',
  emptyFiltered: 'No items match these filters.',
  emptyFilteredHint: 'Try a different search term or clear the selected tags.',
  // Shown instead of the generic empty state when a whole catalog type has
  // no items yet (as opposed to a search/tag filter simply matching
  // nothing) — points people at whichever catalog does have content today
  // rather than leaving them at a dead end.
  emptyByType: {
    product: {
      title: 'Coming soon: executable products',
      description: "We're preparing our first executable products. In the meantime, explore the available resources and templates.",
      cta: 'Explore resources',
    },
    course: {
      title: 'Coming soon: learning paths',
      description: "We're preparing our first courses. In the meantime, explore the available resources and templates.",
      cta: 'Explore resources',
    },
    book: {
      title: 'Coming soon: books and guides',
      description: "We're preparing our first books and technical guides. In the meantime, explore the available resources and templates.",
      cta: 'Explore resources',
    },
    resource: {
      title: 'Coming soon: new resources',
      description: "We're preparing new resources. In the meantime, explore the available products and courses.",
      cta: 'Explore products',
    },
  },
  searchPlaceholder: 'Search by title or category…',
  benefits: 'Benefits',
  requirements: 'Requirements',
  included: 'What is included',
  tags: {
    filterLabel: 'Tags',
    all: 'All',
    clear: 'Clear',
  },
  author: 'Author',
  level: 'Level',
  duration: 'Duration',
  category: 'Category',
  backToCatalog: 'Back to catalog',
  openPreview: 'Open preview',
  requestAccess: 'Request access',
  requestDemo: 'Request demo',
  requestAccessTitle: 'Access request',
  requestDemoTitle: 'Demo request',
  purchaseSuccess: 'Added to your library.',
  resource: {
    viewContent: 'Open resource',
    viewPreview: 'View sample',
    download: 'Download',
    modalTitle: 'Content',
    loadError: 'Could not load the file content.',
    downloadError: 'Could not download the file.',
    truncated: 'The file is too large to display in full here — download it to see the complete content.',
    empty: 'This file is empty.',
    copy: 'Copy',
    copied: 'Copied',
    renderError: 'This file could not be rendered. Try downloading it instead.',
    tooLarge: 'This file is too large to preview in the browser — use the download button instead.',
    xlsxTruncatedRows: 'Showing only the first rows of this sheet — download the file to see all of it.',
    close: 'Close',
    loadErrorHint: 'You can still get the file directly.',
    emptyHint: 'There is nothing to preview yet for this item.',
    kindMarkdown: 'Guide (Markdown)',
    kindDocx: 'Word document',
    kindXlsx: 'Excel spreadsheet',
    kindCode: 'Code / script',
    kindPdf: 'PDF',
    codeTruncatedLines: 'Showing only the first lines of this file — download it to see the rest.',
    previewBadge: 'Preview',
    previewHint: 'You are viewing a sample. Buy this item to unlock the full book.',
    maximize: 'Maximize',
    restore: 'Restore',
    downloadFormat: {
      pdf: 'PDF',
      epub: 'EPUB',
      kindle: 'Kindle',
      zip: 'All formats',
    },
    buyPrinted: 'Buy printed copy',
    buyPrintedComingSoon: 'Coming soon',
    audiobook: 'Audiobook',
    chapters: 'Chapters',
    noChapters: 'This book has no platform-hosted audiobook chapters yet.',
    chapterLoadError: 'Could not load this chapter.',
    previousChapter: 'Previous chapter',
    nextChapter: 'Next chapter',
    formatUnavailable: 'This edition has not been uploaded yet.',
    viewer: {
      toc: 'Index',
      searchPlaceholder: 'Search in this document…',
      clearSearch: 'Clear search',
      noMatches: '0/0',
      prevMatch: 'Previous match',
      nextMatch: 'Next match',
      copySection: 'Copy this section',
      sectionCopied: 'Section copied',
      copySheet: 'Copy sheet',
      sheetCopied: 'Sheet copied',
      searchRowsPlaceholder: 'Filter rows…',
      noRowsMatch: 'No rows match your search.',
      resumedTitle: 'Resumed where you left off',
    },
  },
  description: {
    showMore: 'Show more',
    showLess: 'Show less',
  },
  downloadInfo: {
    title: 'What you get',
    files: 'files',
  },
  gettingStarted: {
    title: 'Getting started',
  },
  version: {
    title: 'Version & compatibility',
    current: 'Current version',
    newVersionAvailable: 'New version available',
    updatedAt: 'Last updated',
    acquiredAt: 'You acquired this on',
    compatibility: 'Compatible with',
    changelog: 'Changelog',
  },
  license: {
    title: 'Usage license',
    scope: 'Permitted use',
    scopeUnspecified: 'Not specified — contact us if you need this clarified before buying.',
    scopeOptions: {
      individual: 'Individual use',
      company: 'Use within a company',
    },
    redistribution: 'Redistribution',
    redistributionUnspecified: 'Not specified — contact us if you need this clarified before buying.',
    redistributionOptions: {
      prohibited: 'Prohibited',
      allowed: 'Allowed',
      allowed_with_attribution: 'Allowed with attribution',
    },
    updatesIncluded: 'Future updates are included',
    updatesNotIncluded: 'Future updates are not included',
    supportIncluded: 'Support is included',
    supportNotIncluded: 'Support is not included',
    refundPolicy: 'Refund policy',
    refundPolicyDefault: 'Covered by our standard digital-content refund policy — see the Terms of Service.',
  },
  levels: {
    beginner: 'Beginner',
    intermediate: 'Intermediate',
    advanced: 'Advanced',
  },
  rating: {
    upvote: 'This helped me',
    downvote: "Didn't help",
    removeMyRating: 'Remove my rating',
    tagsHint: 'What stood out? (up to 3)',
    votePrompt: 'Pick 👍 or 👎 to save your tags',
    sortTopRated: 'Top rated',
    sortNewest: 'Newest',
    infoTooltip:
      '👍 This helped me: this item was useful, well made, or worth it. 👎 Didn\'t help: it didn\'t meet your expectations or wasn\'t useful for you. Your vote is shown as a public count and helps other users and the catalog ranking.',
    tags: {
      saves_time: 'Saves time',
      high_quality: 'High quality',
      easy_to_apply: 'Easy to apply',
      great_support: 'Great support',
      worth_price: 'Worth the price',
      too_technical: 'Too technical',
      outdated: 'Outdated',
      not_worth_price: 'Not worth the price',
    },
  },
  review: {
    title: 'Reviews',
    writeTitle: 'Leave a review',
    ratingLabel: 'Your rating',
    commentPlaceholder: 'Share your experience with this item (optional)',
    submit: 'Submit review',
    update: 'Update review',
    remove: 'Delete my review',
    purchaseRequired: 'Buy this item to leave a review.',
    empty: 'No reviews yet — be the first to write one.',
    average: '{{rating}} out of 5 ({{count}} reviews)',
    averageOne: '{{rating}} out of 5 (1 review)',
    count: '({{count}} reviews)',
    countOne: '(1 review)',
    loadMore: 'Load more reviews',
  },
  share: {
    button: 'Share',
    copied: 'Link copied to clipboard',
    error: 'Could not share this item.',
  },
  pages: {
    products: { title: 'Product catalog', subtitle: 'SaaS tools and platforms for your workflow.' },
    courses: { title: 'Course catalog', subtitle: 'Training paths to level up your team.' },
    books: { title: 'Book catalog', subtitle: 'Technical books and reference guides.' },
    resources: { title: 'Resource catalog', subtitle: 'Templates and downloadable kits.' },
    favorites: { title: 'Favorites', subtitle: 'Items you saved for later.' },
    purchases: { title: 'My purchases', subtitle: 'Everything you have bought.' },
    myProducts: { title: 'My products', subtitle: 'Products in your library.' },
    myCourses: { title: 'My courses', subtitle: 'Courses in your library.' },
    myBooks: { title: 'My books', subtitle: 'Books in your library.' },
    myResources: { title: 'My resources', subtitle: 'Downloadable resources you own.' },
    myLibrary: {
      title: 'My library',
      subtitle: 'Everything you own, in one place — switch tabs to filter by type.',
      tabs: { all: 'All' },
    },
  },
  myPurchases: {
    moreInfo: 'More information',
    hide: 'Hide',
    access: 'Access',
    download: 'Download',
    requestSupport: 'Request support',
    includedItems: 'Included items',
    purchasedAt: 'Purchase date',
    amountPaid: 'Price paid',
    discount: 'Discount applied',
    none: 'None',
    method: 'Acquisition method',
    invoice: 'Invoice',
    viewInvoice: 'View invoice',
    notAvailable: 'Not available',
    paymentStatus: 'Payment status',
    version: 'Version acquired',
    standardVersion: 'Standard',
    organizationVia: 'Via organization',
    loadError: 'Could not load your purchases.',
    empty: 'You have not bought anything yet.',
    emptyHint: 'Everything you buy will show up here with its full purchase detail — date, price, invoice and more.',
    detailError: 'Could not load the detail for this purchase.',
    supportModalTitle: 'Request support',
    supportPlaceholder: 'Describe the issue you are having with this item...',
    supportSent: 'Your request has been sent — we will get back to you soon.',
    supportError: 'Could not send your request. Try again.',
    send: 'Send',
    sending: 'Sending...',
    cancel: 'Cancel',
    source: {
      free: 'Free claim',
      stripe_checkout: 'Card payment (Stripe)',
      plan_entitlement: 'Included in plan',
      admin_grant: 'Granted by an admin',
    },
    paymentStatusLabels: {
      paid: 'Paid',
      unpaid: 'Unpaid',
      no_payment_required: 'No payment required',
      not_applicable: 'Not applicable',
      unknown: 'Unknown',
    },
  },
}

export const adminDashboardEn = {
  title: 'Platform command center',
  subtitle: 'Live metrics across catalog, users, purchases and access requests.',
  heroBadge: 'Super Admin',
  metrics: {
    catalog: 'Catalog items',
    catalogHint: 'Published and draft marketplace entries',
    users: 'Users',
    usersHint: 'Registered platform accounts',
    usersActive: 'Active users',
    purchases: 'Purchases',
    purchasesHint: 'Completed catalog acquisitions',
    revenue: 'Billing',
    revenueHint: 'Sum of purchased item prices',
    requests: 'Pending requests',
    products: 'Products',
    courses: 'Courses',
    books: 'Books',
    resources: 'Resources',
    individualPurchases: 'Individual purchases',
    planSignups: 'New plan signups',
    newUsers: 'New users',
  },
  pulse: { title: 'Catalog pulse' },
  charts: {
    users: 'User growth',
    catalog: 'Catalog evolution',
    purchases: 'Purchase activity',
    revenue: 'Billing trend',
    individualRevenue: 'Individual product revenue',
    planSignups: 'New plan signups',
  },
  trend: {
    thisMonth: 'This month',
    previousMonth: 'Previous month',
    avg6Months: 'Avg. last 6 months',
    toggle1m: '1M',
    toggle6m: '6M',
  },
  comparison: {
    title: 'This month so far',
    subtitle: 'Month-to-date, compared with the same number of days last month — not lifetime totals.',
    new: 'New',
    vsLastMonth: 'vs',
  },
  insights: { catalogMix: 'Platform insights', byType: 'Catalog mix' },
  quickActions: 'Quick actions',
  actions: {
    manageCatalog: 'Manage catalog',
    manageUsers: 'Manage users',
    viewPurchases: 'View purchases',
    reviewRequests: 'Review requests',
    myProfile: 'My profile',
  },
  sections: {
    organizations: { title: 'Organizations by type', subtitle: 'Distribution of tenants across the platform' },
    requests: { title: 'Access requests', subtitle: 'Requests grouped by status' },
    ratings: { title: 'Ratings engagement', subtitle: 'Feedback collected via the catalog rating system' },
    usersByRole: { title: 'Users by role', subtitle: 'Distinct users holding each role' },
    testExecutions: { title: 'Executions per product', subtitle: 'Total test runs triggered for each runnable product' },
  },
  testExecutionsLabels: {
    columns: { product: 'Product', included: 'Included per buyer', total: 'Total runs', lastRun: 'Last run' },
    never: 'Never run',
    empty: 'No test-enabled products yet.',
    loadError: 'Could not load execution stats.',
  },
  requestStatus: {
    pending: 'Pending',
    approved: 'Approved',
    rejected: 'Rejected',
    cancelled: 'Cancelled',
  },
  roleLabels: {
    super_admin: 'Super admin',
    org_owner: 'Org owner',
    org_admin: 'Org admin',
    member: 'Member',
    independent_user: 'Independent user',
    content_creator: 'Content creator',
    viewer: 'Viewer',
  },
  ratingsLabels: {
    total: 'Total ratings',
    upvotes: 'Positive',
    downvotes: 'Negative',
    topTags: 'Most mentioned tags',
    empty: 'No ratings submitted yet',
  },
  emptyOrganizations: 'No organizations yet',
  emptyRequests: 'No requests yet',
  emptyRoles: 'No role assignments yet',
  loadError: 'Could not load platform metrics. Check your connection and try again.',
  retry: 'Retry',
  report: {
    title: 'Analytics summary',
    generatedOn: 'Generated on',
  },
  applicationMap: {
    badge: 'Application map',
    title: 'Application map',
    subtitle: 'A single ASE Core root, branching into organizations (with their members) and unaffiliated individual users.',
    rootLabel: 'ASE Core',
    organizations: 'Organizations',
    organizationsTotal: 'organizations',
    individualUsers: 'Individual users',
    individualUsersTotal: 'individual users',
    membersLabel: 'members',
    noMembers: 'No active members',
    noOrganizations: 'No organizations yet',
    noIndividualUsers: 'No individual users',
    truncatedHint: 'Showing the most recent accounts only',
    loadError: 'Could not load the application map.',
  },
}

export const adminDashboardEs = {
  title: 'Centro de mando de la plataforma',
  subtitle: 'Métricas en vivo del catálogo, usuarios, compras y solicitudes.',
  heroBadge: 'Super Admin',
  metrics: {
    catalog: 'Ítems del catálogo',
    catalogHint: 'Entradas publicadas y borrador',
    users: 'Usuarios',
    usersHint: 'Cuentas registradas',
    usersActive: 'Usuarios activos',
    purchases: 'Compras',
    purchasesHint: 'Adquisiciones completadas',
    revenue: 'Facturación',
    revenueHint: 'Suma de precios de ítems comprados',
    requests: 'Solicitudes pendientes',
    products: 'Productos',
    courses: 'Cursos',
    books: 'Libros',
    resources: 'Recursos',
    individualPurchases: 'Productos individuales',
    planSignups: 'Planes nuevos',
    newUsers: 'Usuarios nuevos',
  },
  pulse: { title: 'Pulso del catálogo' },
  charts: {
    users: 'Evolución de usuarios',
    catalog: 'Evolución del catálogo',
    purchases: 'Actividad de compras',
    revenue: 'Tendencia de facturación',
    individualRevenue: 'Facturación de productos individuales',
    planSignups: 'Planes nuevos',
  },
  trend: {
    thisMonth: 'Este mes',
    previousMonth: 'Mes anterior',
    avg6Months: 'Promedio 6 meses',
    toggle1m: '1M',
    toggle6m: '6M',
  },
  comparison: {
    title: 'Lo que llevamos de mes',
    subtitle: 'Del día 1 hasta hoy, comparado con los mismos días del mes anterior — no son totales históricos.',
    new: 'Nuevo',
    vsLastMonth: 'vs',
  },
  insights: { catalogMix: 'Insights de plataforma', byType: 'Mix del catálogo' },
  quickActions: 'Accesos rápidos',
  actions: {
    manageCatalog: 'Gestionar catálogo',
    manageUsers: 'Gestionar usuarios',
    viewPurchases: 'Ver compras',
    reviewRequests: 'Revisar solicitudes',
    myProfile: 'Mi perfil',
  },
  sections: {
    organizations: { title: 'Organizaciones por tipo', subtitle: 'Distribución de organizaciones en la plataforma' },
    requests: { title: 'Solicitudes de acceso', subtitle: 'Solicitudes agrupadas por estado' },
    ratings: { title: 'Uso del sistema de valoraciones', subtitle: 'Feedback recogido mediante el sistema de valoración del catálogo' },
    usersByRole: { title: 'Usuarios por rol', subtitle: 'Usuarios distintos que ostentan cada rol' },
    testExecutions: { title: 'Ejecuciones por producto', subtitle: 'Total de ejecuciones de test disparadas por cada producto ejecutable' },
  },
  testExecutionsLabels: {
    columns: { product: 'Producto', included: 'Incluidas por comprador', total: 'Ejecuciones totales', lastRun: 'Última ejecución' },
    never: 'Nunca ejecutado',
    empty: 'Todavía no hay productos con test configurado.',
    loadError: 'No se pudieron cargar las estadísticas de ejecuciones.',
  },
  requestStatus: {
    pending: 'Pendientes',
    approved: 'Aprobadas',
    rejected: 'Rechazadas',
    cancelled: 'Canceladas',
  },
  roleLabels: {
    super_admin: 'Super admin',
    org_owner: 'Propietario de organización',
    org_admin: 'Administrador de organización',
    member: 'Miembro',
    independent_user: 'Usuario independiente',
    content_creator: 'Creador de contenido',
    viewer: 'Visualizador',
  },
  ratingsLabels: {
    total: 'Valoraciones totales',
    upvotes: 'Positivas',
    downvotes: 'Negativas',
    topTags: 'Etiquetas más mencionadas',
    empty: 'Todavía no hay valoraciones',
  },
  emptyOrganizations: 'Todavía no hay organizaciones',
  emptyRequests: 'Todavía no hay solicitudes',
  emptyRoles: 'Todavía no hay roles asignados',
  loadError: 'No se pudieron cargar las métricas de la plataforma. Comprueba tu conexión e inténtalo de nuevo.',
  retry: 'Reintentar',
  report: {
    title: 'Resumen de analítica',
    generatedOn: 'Generado el',
  },
  applicationMap: {
    badge: 'Mapa de la aplicación',
    title: 'Mapa de la aplicación',
    subtitle: 'Una única raíz ASE Core, que se ramifica en organizaciones (con sus miembros) y usuarios individuales sin organización.',
    rootLabel: 'ASE Core',
    organizations: 'Organizaciones',
    organizationsTotal: 'organizaciones',
    individualUsers: 'Usuarios individuales',
    individualUsersTotal: 'usuarios individuales',
    membersLabel: 'miembros',
    noMembers: 'Sin miembros activos',
    noOrganizations: 'Todavía no hay organizaciones',
    noIndividualUsers: 'No hay usuarios individuales',
    truncatedHint: 'Mostrando solo las cuentas más recientes',
    loadError: 'No se pudo cargar el mapa de la aplicación.',
  },
}

export const adminCatalogEn = {
  title: 'Catalog management',
  subtitle: 'Create, edit and remove products, courses, books and resources.',
  tabs: {
    items: 'Items',
    categories: 'Categories',
    pricing: 'Pricing engine',
  },
  colTitle: 'Title',
  colType: 'Type',
  colStatus: 'Status',
  colPrice: 'Price',
  colActions: 'Actions',
  publishedCount: 'published',
  create: 'New item',
  edit: 'Edit',
  delete: 'Delete',
  confirmDelete: 'Delete this catalog item? This cannot be undone.',
  tabAll: 'All',
  tabProduct: 'Products',
  tabCourse: 'Courses',
  tabBook: 'Books',
  tabResource: 'Resources',
  searchPlaceholder: 'Search by title…',
  formCreate: 'Create catalog item',
  formEdit: 'Edit catalog item',
  save: 'Save',
  cancel: 'Cancel',
  requiredMark: 'Required fields are marked with *.',
  saveError: 'Could not save the item. Check the highlighted fields.',
  slugExists: 'This slug is already in use — choose another one.',
  slugEditHint: 'Changing this changes the item’s public URL — existing links or bookmarks to the old one will stop working.',
  statusNotifyHint: 'Users are notified only when the item is Published, Coming soon or Request only — not while it stays in Draft.',
  repoRedeemCodeExists: 'This redeem code is already used by another book — choose another one.',
  repoUrlHint: 'Must be the URL of a private GitHub repo (https://github.com/owner/repo). Readers who redeem the code below are automatically invited as a collaborator — the repo does not need to be public.',
  repoRedeemCodeHint: 'The code printed inside the book. Shared by all copies — any registered user who enters it gets the repo link, no purchase required.',
  repoPathHint: 'Path to this item\'s FOLDER inside the shared ASE-Catalog repo, e.g. "resources/deploy-checklist" — the repo itself is configured once on the server, no need to repeat its URL here. That folder must contain a README.md (rendered by the "View content" button) and a packaged .zip (served by the "Download" button); their exact filenames don\'t matter, they\'re found automatically. The repo is kept private — nobody is invited as a collaborator, the platform only reads these two files for whoever owns the item.',
  repoPathHintBook: 'Path to this book\'s FOLDER inside the shared ASE-Catalog repo, e.g. "books/my-book" — and inside THAT, one subfolder per edition: "pdf" (the full book, any file inside), "preview" (a short free sample shown to non-buyers of a priced, unpurchased book — never needed for a free item, which always gets the full "pdf" folder), "epub", "kindle" or "kpf" (either folder name works — any of .mobi/.azw3/.azw/.kpf inside) and "zip" (all formats bundled) for the format-specific download buttons. Filenames inside each subfolder don\'t matter — they\'re found automatically, whatever the admin uploaded.',
  bookRedemptionSection: {
    title: 'Code redemption (private source repo)',
    hint: 'Optional and unrelated to the fields below: lets a reader who enters the redeem code printed inside the book get invited as a collaborator on a private GitHub repo of your choice (e.g. the book\'s companion source code). Leave both fields blank if this book doesn\'t offer that.',
  },
  bookContentSection: {
    title: 'Book content (shared ASE-Catalog repo)',
    hint: 'Powers the "View content"/"Download" buttons on the item page: the full PDF, the free preview, and the epub/Kindle/zip downloads, all read from one folder in the single shared repo already configured on the server — no repository URL needed here.',
  },
  productContentSection: {
    title: 'Downloadable content (optional, shared ASE-Catalog repo)',
    hint: 'Attach a downloadable deliverable to this product — a README, packaged .zip, or a single script/config file — using the same shared repo as resources and books. Powers the "View content"/"Download" buttons on the item page, shown to buyers, owners of a plan that includes this product, or anyone at all if the product is free. Leave blank if this product has no downloadable file (e.g. a pure test-execution framework).',
  },
  testExecutionSection: {
    title: 'Test execution (SaaS API)',
    hint: 'Turns this product into a runnable test-automation framework: buyers get client_id/client_secret API credentials and can trigger runs of the GitHub Actions workflow below, up to the run quota set here. Leave the workflow file blank if this product isn\'t runnable.',
  },
  versionSection: {
    title: 'Version & changelog (resources only)',
    hint: 'Shown to buyers on the item detail page: current version, when it last changed, whether a newer version exists since their purchase, and compatibility with tools like ChatGPT or Claude.',
  },
  licenseSection: {
    title: 'Usage license',
    hint: 'Shown to buyers before they purchase, for every catalog type. Leave the refund policy blank to fall back to the platform\'s standard digital-content refund clause.',
  },
  testRepoUrlHint: 'The GitHub repo hosting the framework\'s code and its .github/workflows — this is ASE\'s own repo, buyers never get direct access to it, only API-triggered runs.',
  testWorkflowFileHint: 'Workflow filename under .github/workflows/, e.g. "run-tests.yml" — must declare "on: workflow_dispatch". Required for this product to be runnable.',
  testIncludedRunsHint: 'How many runs a buyer gets per purchase/plan-inclusion of this product — enforced by the public API before dispatching a new run.',
  testInputSchemaSection: {
    title: 'Buyer variables (workflow_dispatch inputs)',
    hint: 'Declare the inputs your workflow actually needs (e.g. BASE_URL, API_TOKEN). Each "Key" here must exactly match an input name under "on.workflow_dispatch.inputs" in the workflow YAML, or GitHub silently ignores it. Buyers fill in their own values per framework before triggering a run — this is what lets more than one customer run it against their own target environment.',
  },
  addTestVariable: 'Add variable',
  removeTestVariable: 'Remove',
  noTestVariables: 'No variables declared — the workflow will be dispatched with no inputs.',
  testVariableFields: {
    key: 'Key (must match the workflow input name)',
    label: 'Label shown to the buyer',
    type: 'Type',
    required: 'Required',
    description: 'Hint text shown next to the field (optional) — NOT the actual value. Each buyer enters their own real value later, from their "Configure" screen.',
    descriptionPlaceholder: 'e.g. "Your environment\'s base URL" — a hint, not a real URL',
    options: 'Options (comma-separated)',
    optionsPlaceholder: 'e.g. GET, POST, PUT, PATCH, DELETE',
    optionsHint: 'Shown as a dropdown to the buyer. Leave "Required" unchecked to also offer a blank option.',
    default: 'Default value (optional)',
    defaultJsonPlaceholder: 'e.g. {"Content-Type": "application/json"}',
  },
  testVariableTypes: {
    text: 'Text',
    secret: 'Secret (masked)',
    choice: 'Choice (dropdown)',
    json: 'JSON (validated text)',
  },
  audiobookUrlHint: 'External link to the audiobook (Drive, S3, Spotify, a dedicated host...) — not stored in the repo, audiobook files are typically too large for it. If using YouTube, upload it as "Unlisted", not "Private": Private only plays for specific Google accounts you invite one by one, which does not work for a storefront with unknown buyers; Unlisted plays for anyone with the link (not searchable) and is what the platform can actually embed and show only to whoever owns the book. Leave blank if this book has no audio edition.',
  tagsHint: 'Free-form tags used to filter the catalog, e.g. "skill, claude, qa".',
  changelogHint: 'One entry per line — shown to buyers as a bullet list of what changed across versions.',
  compatibilityHint: 'Comma-separated, e.g. "ChatGPT, Claude" — shown to buyers as compatibility badges.',
  licenseScopeHint: 'Which uses are covered — shown to buyers before purchase. Leave both unchecked if this isn\'t specified.',
  licenseRefundPolicyHint: 'Leave blank to show the platform\'s standard digital-content refund clause instead of a per-item one.',
  gettingStartedHint: 'Shown on the item detail page so a buyer isn\'t left to figure out the package on their own — e.g. which file to open first, setup steps, prerequisites.',
  seriesNameHint: 'Optional. Items sharing the exact same series name are grouped together — buyers see their progress through the series and a "next recommended item" on the detail page. Leave blank for a standalone item.',
  seriesOrderHint: 'Position within the series (1, 2, 3...) — decides both display order and which unpurchased item gets recommended next.',
  translationHint: 'Leave blank to auto-translate from the Spanish text on save (via DeepL). Type here only to override the automatic translation.',
  longDescriptionMarkdownHint: 'Supports Markdown (headings, **bold**, lists, links...) — rendered with formatting on the item detail page.',
  markdownToolbar: {
    bold: 'Bold',
    boldPlaceholder: 'bold text',
    italic: 'Italic',
    italicPlaceholder: 'italic text',
    heading: 'Heading',
    bulletList: 'Bullet list',
    numberedList: 'Numbered list',
    link: 'Link',
    linkPlaceholder: 'link text',
    edit: 'Edit',
    preview: 'Preview',
    previewEmpty: 'Nothing to preview yet.',
  },
  translationWarning: {
    title: 'Automatic translation is not configured.',
    body: 'DEEPL_API_KEY is missing on the server — every save copies the Spanish text into the English fields as-is instead of translating it.',
  },
  filters: { allTags: 'All tags', tagsLabel: 'Tags', clearTags: 'Clear' },
  placeholders: {
    tags: 'skill, claude, qa',
    titleEn: 'English title (optional — auto-translated if left blank)',
    shortDescriptionEn: 'English short description (optional)',
    longDescriptionEn: 'English long description (optional)',
    seriesName: 'e.g. "Python for QA — trilogy"',
    changelog: 'One line per version, e.g.\nv1.1.0 — Added Claude compatibility\nv1.0.0 — Initial release',
    compatibility: 'ChatGPT, Claude',
    licenseRefundPolicy: 'Leave blank to use the platform\'s standard refund clause',
    gettingStarted: 'e.g. "1. Extract the zip. 2. Open README.md. 3. Import checklist.xlsx into your test suite."',
  },
  manageCategories: 'Manage categories →',
  customFields: { title: 'Category fields' },
  validation: {
    required: 'This field is required.',
    priceMin: 'Price must be 0 or greater.',
  },
  fields: {
    title: 'Title',
    slug: 'Slug',
    type: 'Type',
    category: 'Category',
    shortDescription: 'Short description',
    longDescription: 'Long description',
    titleEn: 'Title (English)',
    shortDescriptionEn: 'Short description (English)',
    longDescriptionEn: 'Long description (English)',
    imageUrl: 'Cover image URL',
    previewUrl: 'Preview URL',
    audiobookUrl: 'Audiobook URL',
    testRepoUrl: 'Framework repository URL',
    testWorkflowFile: 'Workflow file',
    testIncludedRuns: 'Included runs',
    price: 'Price',
    currency: 'Currency',
    status: 'Status',
    level: 'Level',
    duration: 'Duration',
    author: 'Author',
    photo: 'Product photo',
    repoUrl: 'Repository URL',
    repoRedeemCode: 'Redeem code',
    repoPath: 'Folder path in the repo',
    tags: 'Tags',
    seriesName: 'Series name',
    seriesOrder: 'Order in the series',
    currentVersion: 'Current version',
    changelog: 'Changelog',
    compatibility: 'Compatibility',
    licenseScope: 'Permitted use',
    licenseRedistribution: 'Redistribution',
    licenseUpdatesIncluded: 'Updates included',
    licenseSupportIncluded: 'Support included',
    licenseRefundPolicy: 'Refund policy',
    gettingStarted: 'Getting started',
  },
  licenseScopeOptions: {
    individual: 'Individual use',
    company: 'Use within a company',
  },
  licenseRedistributionOptions: {
    prohibited: 'Redistribution prohibited',
    allowed: 'Redistribution allowed',
    allowed_with_attribution: 'Redistribution allowed with attribution',
  },
  uploadPhoto: 'Upload photo',
  uploadPhotoHint: 'JPEG, PNG or WebP up to 2 MB. Stored in the database.',
  gallery: {
    title: 'Image gallery',
    hint: 'Add more images and pick one as the cover. The rest show as a carousel on the item detail page.',
    createHint: 'Add one or more images and mark one as the cover — they upload once you save the item. The rest show as a carousel on the item detail page.',
    empty: 'No gallery images yet.',
    setCover: 'Set as cover',
    delete: 'Remove image',
    addImages: 'Add images',
    addUrl: 'Add',
    urlPlaceholder: 'Image URL…',
  },
  premium: { badge: 'Catalog ops', viewCards: 'Cards', viewTable: 'Table' },
  status: { published: 'Published', draft: 'Draft', coming_soon: 'Coming soon', request_only: 'Request only' },
  testStats: {
    button: 'Usage stats',
    title: 'Usage stats',
    hint: 'How much this product is actually being run, across every buyer.',
    totalRuns: 'Total runs',
    uniqueUsers: 'Unique users',
    includedRuns: 'Runs included per buyer',
    lastRun: 'Last run',
    never: 'Never run',
    byStatus: 'By status',
    byConclusion: 'By result',
    recentRuns: 'Most recent runs',
    columns: { user: 'User', status: 'Status', conclusion: 'Result', created: 'Created' },
    empty: 'No runs yet for this product.',
    loadError: 'Could not load usage stats.',
    close: 'Close',
  },
}

export const adminCatalogEs = {
  title: 'Gestión del catálogo',
  subtitle: 'Crea, edita y elimina productos, cursos, libros y recursos.',
  tabs: {
    items: 'Ítems',
    categories: 'Categorías',
    pricing: 'Motor de precios',
  },
  colTitle: 'Título',
  colType: 'Tipo',
  colStatus: 'Estado',
  colPrice: 'Precio',
  colActions: 'Acciones',
  publishedCount: 'publicados',
  create: 'Nuevo ítem',
  edit: 'Editar',
  delete: 'Eliminar',
  confirmDelete: '¿Eliminar este ítem del catálogo? No se puede deshacer.',
  tabAll: 'Todos',
  tabProduct: 'Productos',
  tabCourse: 'Cursos',
  tabBook: 'Libros',
  tabResource: 'Recursos',
  searchPlaceholder: 'Buscar por título…',
  formCreate: 'Crear ítem del catálogo',
  formEdit: 'Editar ítem del catálogo',
  save: 'Guardar',
  cancel: 'Cancelar',
  requiredMark: 'Los campos obligatorios están marcados con *.',
  saveError: 'No se pudo guardar el ítem. Revisa los campos marcados.',
  slugExists: 'Este slug ya está en uso — elige otro.',
  slugEditHint: 'Cambiar esto cambia la URL pública del ítem — los enlaces o marcadores antiguos dejarán de funcionar.',
  statusNotifyHint: 'A los usuarios solo les llega notificación cuando el ítem está en Publicado, Próximamente o Solo solicitud — no mientras está en Borrador.',
  repoRedeemCodeExists: 'Este código de canje ya lo usa otro libro — elige otro.',
  repoUrlHint: 'Debe ser la URL de un repositorio privado de GitHub (https://github.com/owner/repo). Al canjear el código de abajo, el lector se invita automáticamente como colaborador — el repo no necesita ser público.',
  repoRedeemCodeHint: 'El código impreso dentro del libro. Es el mismo en todas las copias — cualquier usuario registrado que lo introduzca obtiene el enlace al repo, sin necesidad de haberlo comprado aquí.',
  repoPathHint: 'Ruta de la CARPETA de este ítem dentro del repositorio compartido ASE-Catalog, por ejemplo "resources/deploy-checklist" — el repo en sí se configura una vez en el servidor, no hace falta repetir su URL aquí. Esa carpeta debe contener un README.md (lo muestra el botón "Ver contenido") y un .zip empaquetado (lo sirve el botón "Descargar"); el nombre exacto de cada archivo no importa, se encuentran automáticamente. El repo se mantiene privado — a nadie se le invita como colaborador, la plataforma solo lee estos dos archivos para quien tenga acceso al ítem.',
  repoPathHintBook: 'Ruta de la CARPETA de este libro dentro del repositorio compartido ASE-Catalog, por ejemplo "books/mi-libro" — y DENTRO de esa carpeta, una subcarpeta por edición: "pdf" (el libro completo, cualquier archivo dentro), "preview" (una muestra gratuita que ve quien no ha comprado un producto con precio — nunca hace falta en un ítem gratis, que siempre ve la carpeta "pdf" completa), "epub", "kindle" o "kpf" (vale cualquiera de los dos nombres de carpeta — cualquier .mobi/.azw3/.azw/.kpf dentro) y "zip" (todos los formatos juntos) para los botones de descarga por formato. El nombre de cada archivo dentro de su subcarpeta no importa, se encuentra automáticamente, sea cual sea el que subas.',
  bookRedemptionSection: {
    title: 'Canje de código (repo de código privado)',
    hint: 'Opcional e independiente de los campos de abajo: permite que un lector que introduce el código de canje impreso en el libro se invite como colaborador a un repositorio privado de GitHub que elijas (por ejemplo, el código fuente que acompaña al libro). Deja ambos campos en blanco si este libro no ofrece eso.',
  },
  bookContentSection: {
    title: 'Contenido del libro (repo compartido ASE-Catalog)',
    hint: 'Alimenta los botones "Ver contenido"/"Descargar" de la ficha del ítem: el PDF completo, la vista previa gratuita y las descargas en epub/Kindle/zip, todo leído de una sola carpeta dentro del único repositorio compartido ya configurado en el servidor — aquí no hace falta ninguna URL de repositorio.',
  },
  productContentSection: {
    title: 'Contenido descargable (opcional, repo compartido ASE-Catalog)',
    hint: 'Añade un entregable descargable a este producto — un README, un .zip empaquetado, o un único script/archivo de configuración — usando el mismo repo compartido que recursos y libros. Alimenta los botones "Ver contenido"/"Descargar" de la ficha, visibles para compradores, para quien tenga un plan que incluya este producto, o para cualquiera si el producto es gratuito. Déjalo en blanco si este producto no tiene archivo descargable (por ejemplo, un framework de solo ejecución de tests).',
  },
  testExecutionSection: {
    title: 'Ejecución de tests (API SaaS)',
    hint: 'Convierte este producto en un framework de test-automation ejecutable: los compradores reciben credenciales client_id/client_secret y pueden disparar ejecuciones del workflow de GitHub Actions indicado abajo, hasta el cupo de runs configurado aquí. Deja el archivo de workflow en blanco si este producto no es ejecutable.',
  },
  versionSection: {
    title: 'Versión y changelog (solo recursos)',
    hint: 'Se muestra a los compradores en la ficha del ítem: versión actual, cuándo cambió por última vez, si hay una versión más nueva desde su compra, y compatibilidad con herramientas como ChatGPT o Claude.',
  },
  licenseSection: {
    title: 'Licencia de uso',
    hint: 'Se muestra a los compradores antes de comprar, para cualquier tipo de ítem. Deja la política de reembolso en blanco para usar la cláusula estándar de reembolso de contenido digital de la plataforma.',
  },
  testRepoUrlHint: 'El repositorio de GitHub que aloja el código del framework y sus .github/workflows — es un repo propio de ASE, los compradores nunca acceden a él directamente, solo disparan ejecuciones vía API.',
  testWorkflowFileHint: 'Nombre del archivo de workflow dentro de .github/workflows/, por ejemplo "run-tests.yml" — debe declarar "on: workflow_dispatch". Obligatorio para que este producto sea ejecutable.',
  testIncludedRunsHint: 'Cuántas ejecuciones recibe un comprador por cada compra/inclusión en plan de este producto — se aplica en la API pública antes de disparar una nueva ejecución.',
  testInputSchemaSection: {
    title: 'Variables del comprador (inputs de workflow_dispatch)',
    hint: 'Declara los inputs que realmente necesita tu workflow (por ejemplo BASE_URL, API_TOKEN). Cada "Clave" debe coincidir exactamente con un nombre de input bajo "on.workflow_dispatch.inputs" en el YAML del workflow, o GitHub lo ignorará silenciosamente. Cada comprador rellena sus propios valores por framework antes de disparar una ejecución — esto es lo que permite que más de un cliente lo ejecute contra su propio entorno.',
  },
  addTestVariable: 'Añadir variable',
  removeTestVariable: 'Quitar',
  noTestVariables: 'No hay variables declaradas — el workflow se disparará sin inputs.',
  testVariableFields: {
    key: 'Clave (debe coincidir con el nombre del input del workflow)',
    label: 'Etiqueta mostrada al comprador',
    type: 'Tipo',
    required: 'Obligatorio',
    description: 'Texto de ayuda junto al campo (opcional) — NO es el valor real. Cada comprador introduce su propio valor después, desde su pantalla "Configurar".',
    descriptionPlaceholder: 'ej. "URL base de tu entorno" — es una pista, no una URL real',
    options: 'Opciones (separadas por comas)',
    optionsPlaceholder: 'ej. GET, POST, PUT, PATCH, DELETE',
    optionsHint: 'Se muestra como desplegable al comprador. Deja "Obligatorio" sin marcar para ofrecer también una opción en blanco.',
    default: 'Valor por defecto (opcional)',
    defaultJsonPlaceholder: 'ej. {"Content-Type": "application/json"}',
  },
  testVariableTypes: {
    text: 'Texto',
    secret: 'Secreto (oculto)',
    choice: 'Opción (desplegable)',
    json: 'JSON (texto validado)',
  },
  audiobookUrlHint: 'Enlace externo al audiolibro (Drive, S3, Spotify, un hosting dedicado...) — no se guarda en el repo, los archivos de audiolibro suelen pesar demasiado para eso. Si usas YouTube, súbelo como "No listado", no como "Privado": Privado solo lo reproducen las cuentas de Google que invites una a una, lo cual no funciona para una tienda con compradores desconocidos; No listado lo reproduce cualquiera con el enlace (no aparece en buscadores) y es lo que la plataforma puede realmente incrustar y mostrar solo a quien haya comprado el libro. Déjalo en blanco si este libro no tiene edición en audio.',
  tagsHint: 'Etiquetas libres para filtrar el catálogo, por ejemplo "skill, claude, qa".',
  changelogHint: 'Una entrada por línea — se muestra a los compradores como una lista de cambios entre versiones.',
  compatibilityHint: 'Separado por comas, ej. "ChatGPT, Claude" — se muestra a los compradores como etiquetas de compatibilidad.',
  licenseScopeHint: 'Qué usos están cubiertos — se muestra a los compradores antes de comprar. Deja ambos sin marcar si no está especificado.',
  licenseRefundPolicyHint: 'Déjalo en blanco para mostrar la cláusula estándar de reembolso de contenido digital de la plataforma en lugar de una propia del ítem.',
  gettingStartedHint: 'Se muestra en la ficha del ítem para que el comprador no tenga que averiguar por su cuenta cómo usar el paquete — por ejemplo, qué archivo abrir primero, pasos de configuración, requisitos previos.',
  seriesNameHint: 'Opcional. Los ítems con el mismo nombre de serie (exacto) se agrupan: los compradores ven su progreso a través de la serie y un "siguiente recomendado" en la ficha del ítem. Déjalo en blanco para un ítem independiente.',
  seriesOrderHint: 'Posición dentro de la serie (1, 2, 3...) — decide tanto el orden de visualización como qué ítem sin comprar se recomienda a continuación.',
  translationHint: 'Déjalo en blanco para traducir automáticamente el texto en español al guardar (vía DeepL). Escribe aquí solo para anular la traducción automática.',
  longDescriptionMarkdownHint: 'Admite Markdown (títulos, **negrita**, listas, enlaces...) — se muestra con formato en el detalle del ítem.',
  markdownToolbar: {
    bold: 'Negrita',
    boldPlaceholder: 'texto en negrita',
    italic: 'Cursiva',
    italicPlaceholder: 'texto en cursiva',
    heading: 'Título',
    bulletList: 'Lista con viñetas',
    numberedList: 'Lista numerada',
    link: 'Enlace',
    linkPlaceholder: 'texto del enlace',
    edit: 'Editar',
    preview: 'Vista previa',
    previewEmpty: 'Todavía no hay nada que previsualizar.',
  },
  translationWarning: {
    title: 'La traducción automática no está configurada.',
    body: 'Falta DEEPL_API_KEY en el servidor — cada guardado copia el texto en español a los campos en inglés tal cual, sin traducirlo.',
  },
  filters: { allTags: 'Todas las etiquetas', tagsLabel: 'Etiquetas', clearTags: 'Limpiar' },
  placeholders: {
    tags: 'skill, claude, qa',
    titleEn: 'Título en inglés (opcional — se traduce solo si se deja en blanco)',
    shortDescriptionEn: 'Descripción corta en inglés (opcional)',
    longDescriptionEn: 'Descripción larga en inglés (opcional)',
    seriesName: 'ej. "Python para QA — trilogía"',
    changelog: 'Una línea por versión, ej.\nv1.1.0 — Añadida compatibilidad con Claude\nv1.0.0 — Versión inicial',
    compatibility: 'ChatGPT, Claude',
    licenseRefundPolicy: 'Déjalo en blanco para usar la cláusula de reembolso estándar de la plataforma',
    gettingStarted: 'ej. "1. Descomprime el zip. 2. Abre README.md. 3. Importa checklist.xlsx en tu suite de pruebas."',
  },
  manageCategories: 'Gestionar categorías →',
  customFields: { title: 'Campos de la categoría' },
  validation: {
    required: 'Este campo es obligatorio.',
    priceMin: 'El precio debe ser 0 o mayor.',
  },
  fields: {
    title: 'Título',
    slug: 'Slug',
    type: 'Tipo',
    category: 'Categoría',
    shortDescription: 'Descripción corta',
    longDescription: 'Descripción larga',
    titleEn: 'Título (inglés)',
    shortDescriptionEn: 'Descripción corta (inglés)',
    longDescriptionEn: 'Descripción larga (inglés)',
    imageUrl: 'URL imagen de portada',
    previewUrl: 'URL de vista previa',
    audiobookUrl: 'URL del audiolibro',
    testRepoUrl: 'URL del repositorio del framework',
    testWorkflowFile: 'Archivo de workflow',
    testIncludedRuns: 'Runs incluidos',
    price: 'Precio',
    currency: 'Moneda',
    status: 'Estado',
    level: 'Nivel',
    duration: 'Duración',
    author: 'Autor',
    photo: 'Foto del producto',
    repoUrl: 'URL del repositorio',
    repoRedeemCode: 'Código de canje',
    repoPath: 'Ruta de la carpeta en el repo',
    tags: 'Etiquetas',
    seriesName: 'Nombre de la serie',
    seriesOrder: 'Orden en la serie',
    currentVersion: 'Versión actual',
    changelog: 'Historial de cambios',
    compatibility: 'Compatibilidad',
    licenseScope: 'Uso permitido',
    licenseRedistribution: 'Redistribución',
    licenseUpdatesIncluded: 'Actualizaciones incluidas',
    licenseSupportIncluded: 'Soporte incluido',
    licenseRefundPolicy: 'Política de reembolso',
    gettingStarted: 'Cómo empezar',
  },
  licenseScopeOptions: {
    individual: 'Uso individual',
    company: 'Uso dentro de una empresa',
  },
  licenseRedistributionOptions: {
    prohibited: 'Redistribución prohibida',
    allowed: 'Redistribución permitida',
    allowed_with_attribution: 'Redistribución permitida con atribución',
  },
  uploadPhoto: 'Subir foto',
  uploadPhotoHint: 'JPEG, PNG o WebP hasta 2 MB. Se guarda en la base de datos.',
  gallery: {
    title: 'Galería de imágenes',
    hint: 'Añade más imágenes y elige una como portada. El resto se muestra como carrusel en el detalle del ítem.',
    createHint: 'Añade una o varias imágenes y marca una como portada — se suben al guardar el ítem. El resto se muestra como carrusel en el detalle del ítem.',
    empty: 'Todavía no hay imágenes en la galería.',
    setCover: 'Marcar como portada',
    delete: 'Quitar imagen',
    addImages: 'Añadir imágenes',
    addUrl: 'Añadir',
    urlPlaceholder: 'URL de la imagen…',
  },
  premium: { badge: 'Operaciones catálogo', viewCards: 'Tarjetas', viewTable: 'Tabla' },
  status: { published: 'Publicado', draft: 'Borrador', coming_soon: 'Próximamente', request_only: 'Solo solicitud' },
  testStats: {
    button: 'Estadísticas de uso',
    title: 'Estadísticas de uso',
    hint: 'Cuánto se está usando realmente este producto, entre todos los compradores.',
    totalRuns: 'Ejecuciones totales',
    uniqueUsers: 'Usuarios distintos',
    includedRuns: 'Ejecuciones incluidas por comprador',
    lastRun: 'Última ejecución',
    never: 'Nunca ejecutado',
    byStatus: 'Por estado',
    byConclusion: 'Por resultado',
    recentRuns: 'Ejecuciones más recientes',
    columns: { user: 'Usuario', status: 'Estado', conclusion: 'Resultado', created: 'Creada' },
    empty: 'Todavía no hay ejecuciones de este producto.',
    loadError: 'No se pudieron cargar las estadísticas de uso.',
    close: 'Cerrar',
  },
}

export const adminCatalogCategoriesEn = {
  premium: { badge: 'Catalog structure' },
  title: 'Catalog categories',
  subtitle: 'Manage the categories catalog items can use, and the custom fields each one collects.',
  create: 'New category',
  edit: 'Edit',
  delete: 'Delete',
  cancel: 'Cancel',
  save: 'Save',
  saving: 'Saving…',
  saveError: 'Could not save the category.',
  loadError: 'Could not load the categories.',
  empty: 'No categories yet',
  active: 'Active',
  inactive: 'Inactive',
  noFields: 'No custom fields',
  confirmDelete: 'This category will be permanently deleted. Existing items keep their category text either way.',
  newTitle: 'New category',
  editTitle: 'Edit category',
  addField: 'Add field',
  removeField: 'Remove',
  questionnaireHint: 'These fields appear on the admin item-creation form whenever this category is selected, and their answers are saved per item.',
  fieldTypes: { text: 'Text', textarea: 'Long text', number: 'Number', boolean: 'Yes/No', url: 'URL', select: 'Choice list' },
  fields: {
    name: 'Name',
    slug: 'Slug',
    description: 'Description',
    active: 'Active',
    questionnaire: 'Custom fields',
    fieldLabel: 'Field label',
    optionsPlaceholder: 'option 1, option 2, option 3',
    required: 'Required',
  },
}

export const adminCatalogCategoriesEs = {
  premium: { badge: 'Estructura de catálogo' },
  title: 'Categorías del catálogo',
  subtitle: 'Gestiona las categorías que pueden usar los ítems del catálogo y los campos personalizados que recoge cada una.',
  create: 'Nueva categoría',
  edit: 'Editar',
  delete: 'Eliminar',
  cancel: 'Cancelar',
  save: 'Guardar',
  saving: 'Guardando…',
  saveError: 'No se pudo guardar la categoría.',
  loadError: 'No se pudieron cargar las categorías.',
  empty: 'Todavía no hay categorías',
  active: 'Activa',
  inactive: 'Inactiva',
  noFields: 'Sin campos personalizados',
  confirmDelete: 'Esta categoría se eliminará de forma permanente. Los ítems existentes conservan su texto de categoría de todas formas.',
  newTitle: 'Nueva categoría',
  editTitle: 'Editar categoría',
  addField: 'Añadir campo',
  removeField: 'Quitar',
  questionnaireHint: 'Estos campos aparecen en el formulario de creación de ítems del admin cuando se selecciona esta categoría, y sus respuestas se guardan por ítem.',
  fieldTypes: { text: 'Texto', textarea: 'Texto largo', number: 'Número', boolean: 'Sí/No', url: 'URL', select: 'Lista de opciones' },
  fields: {
    name: 'Nombre',
    slug: 'Slug',
    description: 'Descripción',
    active: 'Activa',
    questionnaire: 'Campos personalizados',
    fieldLabel: 'Etiqueta del campo',
    optionsPlaceholder: 'opción 1, opción 2, opción 3',
    required: 'Obligatorio',
  },
}

export const adminPurchasesEn = {
  title: 'Purchases',
  subtitle: 'Billing totals, purchase history and top buyers.',
  tabs: {
    items: 'Individual products',
    plans: 'Plans',
  },
  colUser: 'User',
  colItem: 'Item',
  colType: 'Type',
  colSource: 'Source',
  colDate: 'Date',
  colOrg: 'Organization',
  colOwner: 'Owner',
  colPlan: 'Plan',
  colPrice: 'Price',
  colStatus: 'Status',
  colTenure: 'Tenure',
  colStarted: 'Started',
  months: 'mo.',
  totalPurchases: 'Total purchases',
  totalPlanSubscriptions: 'Active plan subscriptions',
  totalRevenue: 'Total billing',
  topUsers: 'Top 5 buyers',
  purchasesCount: 'purchases',
  insights: 'Purchase insights',
  recent: 'Recent purchases',
  emptyItems: 'No individual purchases match these filters.',
  emptyPlans: 'No plan subscriptions match these filters.',
  sourceLabels: {
    free: 'Free',
    stripe_checkout: 'Stripe',
    admin_grant: 'Admin grant',
  },
  statusLabels: {
    trialing: 'Trialing',
    active: 'Active',
    past_due: 'Past due',
    canceled: 'Canceled',
    expired: 'Expired',
  },
  filters: {
    search: 'Search user or item…',
    dateFrom: 'From',
    dateTo: 'To',
    clear: 'Clear filters',
  },
}

export const adminAuditLogEn = {
  heroBadge: 'Activity log',
  title: 'Platform activity log',
  subtitle: 'Who changed what, and when — catalog edits, user management, and access request reviews.',
  loadError: 'Could not load the activity log.',
  empty: 'No activity recorded yet',
  emptyHint: 'Actions like editing the catalog, managing users or reviewing requests will show up here.',
  filters: {
    entityType: 'Entity',
    action: 'Action',
    from: 'From',
    to: 'To',
    all: 'All',
  },
  columns: {
    date: 'Date',
    actor: 'Actor',
    action: 'Action',
    entity: 'Entity',
    details: 'Details',
  },
}

export const adminAuditLogEs = {
  heroBadge: 'Registro de actividad',
  title: 'Registro de actividad de la plataforma',
  subtitle: 'Quién cambió qué y cuándo — ediciones de catálogo, gestión de usuarios y revisión de solicitudes.',
  loadError: 'No se pudo cargar el registro de actividad.',
  empty: 'Todavía no hay actividad registrada',
  emptyHint: 'Acciones como editar el catálogo, gestionar usuarios o revisar solicitudes aparecerán aquí.',
  filters: {
    entityType: 'Entidad',
    action: 'Acción',
    from: 'Desde',
    to: 'Hasta',
    all: 'Todas',
  },
  columns: {
    date: 'Fecha',
    actor: 'Actor',
    action: 'Acción',
    entity: 'Entidad',
    details: 'Detalles',
  },
}

export const adminBookRedemptionsEn = {
  heroBadge: 'Book redemptions',
  title: 'Book code redemptions',
  subtitle: 'Everyone who has redeemed a code from your books to access a private repo — signed-in and anonymous.',
  loadError: 'Could not load book redemptions.',
  empty: 'No redemptions yet',
  emptyHint: 'When a reader redeems a code, it will show up here.',
  filters: {
    search: 'Search user, book or GitHub username…',
    dateFrom: 'From',
    dateTo: 'To',
    clear: 'Clear filters',
  },
  columns: {
    date: 'Date',
    user: 'User',
    book: 'Book',
    githubUsername: 'GitHub username',
  },
  anonymous: 'Anonymous',
}

export const adminBookRedemptionsEs = {
  heroBadge: 'Canjeos de libros',
  title: 'Canjeos de código de libros',
  subtitle: 'Todos los que han canjeado un código de tus libros para acceder a un repo privado — con y sin sesión iniciada.',
  loadError: 'No se pudieron cargar los canjeos.',
  empty: 'Todavía no hay canjeos',
  emptyHint: 'Cuando un lector canjee un código, aparecerá aquí.',
  filters: {
    search: 'Buscar usuario, libro o usuario de GitHub…',
    dateFrom: 'Desde',
    dateTo: 'Hasta',
    clear: 'Limpiar filtros',
  },
  columns: {
    date: 'Fecha',
    user: 'Usuario',
    book: 'Libro',
    githubUsername: 'Usuario de GitHub',
  },
  anonymous: 'Anónimo',
}

export const adminAnnouncementsEn = {
  heroBadge: 'Announcements',
  title: 'Broadcast an announcement',
  subtitle: 'Send an in-app notification to every user on the platform (independent users and organization members).',
  fields: { title: 'Title', body: 'Message (optional)', link: 'Link (optional)' },
  placeholders: {
    title: 'e.g. Scheduled maintenance this weekend',
    body: 'Add more detail for readers…',
    link: 'https://…',
  },
  send: 'Send announcement',
  sending: 'Sending…',
  error: 'Could not send the announcement. Try again.',
  aboutTitle: 'How it works',
  aboutBody: 'Every non-superadmin account receives an in-app notification instantly. Super admins are never included.',
  recipientsSent: 'Sent to {{count}} users',
}

export const adminAnnouncementsEs = {
  heroBadge: 'Anuncios',
  title: 'Enviar un anuncio',
  subtitle: 'Envía una notificación dentro de la app a todos los usuarios de la plataforma (usuarios independientes y miembros de organizaciones).',
  fields: { title: 'Título', body: 'Mensaje (opcional)', link: 'Enlace (opcional)' },
  placeholders: {
    title: 'ej. Mantenimiento programado este fin de semana',
    body: 'Añade más detalle para los lectores…',
    link: 'https://…',
  },
  send: 'Enviar anuncio',
  sending: 'Enviando…',
  error: 'No se pudo enviar el anuncio. Inténtalo de nuevo.',
  aboutTitle: 'Cómo funciona',
  aboutBody: 'Cada cuenta que no sea super admin recibe una notificación al instante. Los super admins nunca se incluyen.',
  recipientsSent: 'Enviado a {{count}} usuarios',
}

export const adminSystemEn = {
  heroBadge: 'System',
  title: 'System',
  subtitle: 'Status, error logs, data reset, and demo data — all in one place.',
  tabs: {
    status: 'Status',
    errors: 'Error logs',
    reset: 'Data reset',
    demo: 'Demo data',
  },
}

export const adminSystemStatusEn = {
  heroBadge: 'System status',
  title: 'System status',
  subtitle: 'A live, read-only snapshot of the API, database, and integrations.',
  loadError: 'Could not load system status.',
  refresh: 'Refresh',
  lastChecked: 'Last checked',
  status: { ok: 'Operational', error: 'Error', configured: 'Configured', notConfigured: 'Not configured', enabled: 'Enabled', disabled: 'Disabled' },
  cards: {
    api: 'API',
    apiHint: 'Uptime',
    database: 'Database',
    databaseHint: 'Query latency',
    githubIntegration: 'GitHub integration',
    githubIntegrationHint: 'Book repo auto-invite',
    rateLimiting: 'Rate limiting',
    rateLimitingHint: 'Public/sensitive endpoints',
    smtp: 'Email (SMTP)',
    smtpHint: 'Transactional emails',
    sentry: 'Error monitoring',
    sentryHint: 'Sentry',
    redis: 'Rate-limiter storage',
    redisHint: 'Redis (falls back to in-memory)',
    environment: 'Environment',
    mvpMode: 'MVP mode',
  },
  counts: {
    title: 'Platform counts',
    users: 'Users',
    catalog: 'Catalog items',
    requestsPending: 'Pending requests',
  },
  adoption: {
    title: 'Adoption',
    emailVerified: 'Email verified',
    twoFactor: '2FA enabled',
  },
  errors: {
    title: 'Unhandled errors',
    last24h: 'Last 24h',
    last7d: 'Last 7 days',
    viewAll: 'View full log',
  },
  scheduler: {
    title: 'Background scheduler',
    empty: 'No jobs registered.',
    nextRun: 'Next run',
    noNextRun: 'Not scheduled',
  },
  lifecycle: {
    title: 'Account lifecycle policy',
    runNow: 'Run sweep now',
    running: 'Running…',
    policyHint:
      '2FA grace period: {{grace}} days · Inactivity suspension: {{inactivity}} days · Deletion after suspension: {{delete}} days',
    pendingTwoFactor: 'Pending 2FA activation',
    suspendedTwoFactor: 'Suspended (no 2FA)',
    suspendedInactivity: 'Suspended (inactivity)',
    pendingDeletion: 'Pending deletion soon',
    sweepResult: 'Sweep complete — suspended {{suspended2fa}} for missing 2FA, {{suspendedInactivity}} for inactivity, deleted {{deleted}}.',
  },
}

export const adminErrorLogsEn = {
  heroBadge: 'Error logs',
  title: 'Error logs',
  subtitle: 'Unhandled backend exceptions, captured automatically — a self-hosted stand-in for a third-party error monitor.',
  loadError: 'Could not load error logs.',
  empty: 'No errors logged',
  emptyHint: 'Nothing has crashed yet in the selected range — good sign.',
  viewDetail: 'View',
  close: 'Close',
  filters: {
    errorType: 'Error type',
    path: 'Path',
    method: 'Method',
    all: 'All',
    from: 'From',
    to: 'To',
  },
  columns: {
    date: 'Date',
    method: 'Method',
    path: 'Path',
    errorType: 'Error type',
    user: 'User',
    actions: 'Actions',
  },
  detail: {
    message: 'Message',
    ip: 'IP address',
    traceback: 'Traceback',
  },
}

export const adminErrorLogsEs = {
  heroBadge: 'Registro de errores',
  title: 'Registro de errores',
  subtitle: 'Excepciones no controladas del backend, capturadas automáticamente — un sustituto propio de un monitor de errores de terceros.',
  loadError: 'No se pudo cargar el registro de errores.',
  empty: 'No hay errores registrados',
  emptyHint: 'Nada se ha caído todavía en el rango seleccionado — buena señal.',
  viewDetail: 'Ver',
  close: 'Cerrar',
  filters: {
    errorType: 'Tipo de error',
    path: 'Ruta',
    method: 'Método',
    all: 'Todos',
    from: 'Desde',
    to: 'Hasta',
  },
  columns: {
    date: 'Fecha',
    method: 'Método',
    path: 'Ruta',
    errorType: 'Tipo de error',
    user: 'Usuario',
    actions: 'Acciones',
  },
  detail: {
    message: 'Mensaje',
    ip: 'Dirección IP',
    traceback: 'Traza (traceback)',
  },
}

export const adminDataResetEn = {
  heroBadge: 'Danger zone',
  title: 'Data reset',
  subtitle:
    'Wipe platform data before launch. Every action here is permanent and only ever preserves your own super admin account, its organization and role.',
  loadError: 'Could not load the data domains.',
  masterTitle: 'Delete everything',
  masterSubtitle:
    'Wipes every table listed below in one go. Only your user, your organization and your super_admin role survive.',
  masterButton: 'Delete everything except me',
  rowsLabel: 'rows',
  extraTablesLabel: 'Also wipes:',
  specialOrgHint: 'Removes every organization except yours (and everything tied only to those organizations).',
  specialUserHint: 'Removes every user except you.',
  deleteDomainButton: 'Delete',
  modalTitleDomain: 'Delete',
  modalTitleAll: 'Delete everything',
  typePhraseLabel: 'Type the following phrase exactly to confirm:',
  phraseInputPlaceholder: 'Type the confirmation phrase',
  passwordLabel: 'Your password',
  passwordPlaceholder: 'Confirm your password',
  cancel: 'Cancel',
  confirmDelete: 'Confirm deletion',
  deleting: 'Deleting…',
  successPrefix: 'Done —',
  errorPrefix: 'Could not complete the action:',
  preservedNotePrefix: 'Only this account will remain:',
}

export const adminDataResetEs = {
  heroBadge: 'Zona de peligro',
  title: 'Reinicio de datos',
  subtitle:
    'Vacía los datos de la plataforma antes del lanzamiento. Toda acción aquí es permanente y siempre conserva únicamente tu propia cuenta de super admin, su organización y su rol.',
  loadError: 'No se pudieron cargar los dominios de datos.',
  masterTitle: 'Eliminar todo',
  masterSubtitle:
    'Vacía de una vez todas las tablas listadas abajo. Solo sobreviven tu usuario, tu organización y tu rol de super_admin.',
  masterButton: 'Eliminar todo excepto a mí',
  rowsLabel: 'filas',
  extraTablesLabel: 'También vacía:',
  specialOrgHint: 'Elimina todas las organizaciones excepto la tuya (y todo lo que dependa solo de esas organizaciones).',
  specialUserHint: 'Elimina todos los usuarios excepto tú.',
  deleteDomainButton: 'Eliminar',
  modalTitleDomain: 'Eliminar',
  modalTitleAll: 'Eliminar todo',
  typePhraseLabel: 'Escribe exactamente la siguiente frase para confirmar:',
  phraseInputPlaceholder: 'Escribe la frase de confirmación',
  passwordLabel: 'Tu contraseña',
  passwordPlaceholder: 'Confirma tu contraseña',
  cancel: 'Cancelar',
  confirmDelete: 'Confirmar eliminación',
  deleting: 'Eliminando…',
  successPrefix: 'Listo —',
  errorPrefix: 'No se pudo completar la acción:',
  preservedNotePrefix: 'Solo permanecerá esta cuenta:',
}

export const adminDemoDataEn = {
  heroBadge: 'Demo data',
  title: 'Paid & free user preview',
  subtitle: 'Create throwaway test accounts — some with an active paid subscription, one plain independent account with no plan — so you can log in and see exactly what each kind of user sees, upsell included.',
  actionTitle: 'Create demo Pro / Business / Free accounts',
  actionSubtitle: 'Safe to run more than once — it never duplicates accounts, only tops up anything missing (e.g. a plan\'s included items changed since last run).',
  createButton: 'Create demo accounts',
  creating: 'Creating…',
  error: 'Could not create the demo accounts.',
  resultTitle: 'Demo accounts ready',
  passwordLabel: 'Password for every demo account:',
  columns: { email: 'Email', plan: 'Plan', itemsGranted: 'Catalog items granted', status: 'Status' },
  badges: { created: 'Created', existing: 'Already existed' },
  freePlan: 'No plan',
}

export const adminDemoDataEs = {
  heroBadge: 'Datos de demo',
  title: 'Vista previa de usuario pagado y gratuito',
  subtitle: 'Crea cuentas de prueba desechables — algunas con una suscripción de pago activa, y una cuenta independiente sin plan — para que puedas entrar y ver exactamente lo que ve cada tipo de usuario, incluida la invitación a suscribirse.',
  actionTitle: 'Crear cuentas de demo Pro / Business / Gratuita',
  actionSubtitle: 'Se puede ejecutar más de una vez sin problema — nunca duplica cuentas, solo completa lo que falte (por ejemplo, si cambiaste qué incluye un plan desde la última vez).',
  createButton: 'Crear cuentas de demo',
  creating: 'Creando…',
  error: 'No se pudieron crear las cuentas de demo.',
  resultTitle: 'Cuentas de demo listas',
  passwordLabel: 'Contraseña para todas las cuentas de demo:',
  columns: { email: 'Email', plan: 'Plan', itemsGranted: 'Elementos de catálogo concedidos', status: 'Estado' },
  badges: { created: 'Creada', existing: 'Ya existía' },
  freePlan: 'Sin plan',
}

export const adminSystemEs = {
  heroBadge: 'Sistema',
  title: 'Sistema',
  subtitle: 'Estado, registro de errores, reinicio de datos y datos de demo — todo en un solo lugar.',
  tabs: {
    status: 'Estado',
    errors: 'Registro de errores',
    reset: 'Reinicio de datos',
    demo: 'Datos de demo',
  },
}

export const adminSystemStatusEs = {
  heroBadge: 'Estado del sistema',
  title: 'Estado del sistema',
  subtitle: 'Una foto en vivo, de solo lectura, de la API, la base de datos y las integraciones.',
  loadError: 'No se pudo cargar el estado del sistema.',
  refresh: 'Actualizar',
  lastChecked: 'Última comprobación',
  status: { ok: 'Operativo', error: 'Error', configured: 'Configurado', notConfigured: 'Sin configurar', enabled: 'Activado', disabled: 'Desactivado' },
  cards: {
    api: 'API',
    apiHint: 'Tiempo activo',
    database: 'Base de datos',
    databaseHint: 'Latencia de consulta',
    githubIntegration: 'Integración con GitHub',
    githubIntegrationHint: 'Invitación automática a repos de libros',
    rateLimiting: 'Límite de peticiones',
    rateLimitingHint: 'Endpoints públicos/sensibles',
    smtp: 'Correo (SMTP)',
    smtpHint: 'Emails transaccionales',
    sentry: 'Monitorización de errores',
    sentryHint: 'Sentry',
    redis: 'Almacén del limitador de peticiones',
    redisHint: 'Redis (usa memoria como respaldo)',
    environment: 'Entorno',
    mvpMode: 'Modo MVP',
  },
  counts: {
    title: 'Recuentos de la plataforma',
    users: 'Usuarios',
    catalog: 'Artículos del catálogo',
    requestsPending: 'Solicitudes pendientes',
  },
  adoption: {
    title: 'Adopción',
    emailVerified: 'Email verificado',
    twoFactor: '2FA activado',
  },
  errors: {
    title: 'Errores no controlados',
    last24h: 'Últimas 24h',
    last7d: 'Últimos 7 días',
    viewAll: 'Ver registro completo',
  },
  scheduler: {
    title: 'Tareas programadas',
    empty: 'No hay tareas registradas.',
    nextRun: 'Próxima ejecución',
    noNextRun: 'Sin programar',
  },
  lifecycle: {
    title: 'Política del ciclo de vida de cuentas',
    runNow: 'Ejecutar ahora',
    running: 'Ejecutando…',
    policyHint:
      'Plazo de gracia 2FA: {{grace}} días · Suspensión por inactividad: {{inactivity}} días · Eliminación tras suspensión: {{delete}} días',
    pendingTwoFactor: 'Pendientes de activar 2FA',
    suspendedTwoFactor: 'Suspendidas (sin 2FA)',
    suspendedInactivity: 'Suspendidas (inactividad)',
    pendingDeletion: 'Eliminación próxima',
    sweepResult: 'Barrido completo — {{suspended2fa}} suspendidas por falta de 2FA, {{suspendedInactivity}} por inactividad, {{deleted}} eliminadas.',
  },
}

export const adminSearchEn = {
  placeholder: 'Search users, catalog…',
  searching: 'Searching…',
  noResults: 'No results',
  groupUsers: 'Users',
  groupCatalog: 'Catalog items',
}

export const adminSearchEs = {
  placeholder: 'Buscar usuarios, catálogo…',
  searching: 'Buscando…',
  noResults: 'Sin resultados',
  groupUsers: 'Usuarios',
  groupCatalog: 'Artículos del catálogo',
}

export const adminPurchasesEs = {
  title: 'Compras',
  subtitle: 'Totales de facturación, historial y mejores compradores.',
  tabs: {
    items: 'Productos individuales',
    plans: 'Planes',
  },
  colUser: 'Usuario',
  colItem: 'Ítem',
  colType: 'Tipo',
  colSource: 'Origen',
  colDate: 'Fecha',
  colOrg: 'Organización',
  colOwner: 'Propietario',
  colPlan: 'Plan',
  colPrice: 'Precio',
  colStatus: 'Estado',
  colTenure: 'Antigüedad',
  colStarted: 'Inicio',
  months: 'meses',
  totalPurchases: 'Compras totales',
  totalPlanSubscriptions: 'Suscripciones a planes activas',
  totalRevenue: 'Facturación total',
  topUsers: 'Top 5 compradores',
  purchasesCount: 'compras',
  insights: 'Insights de compras',
  recent: 'Compras recientes',
  emptyItems: 'No hay compras individuales con estos filtros.',
  emptyPlans: 'No hay suscripciones a planes con estos filtros.',
  sourceLabels: {
    free: 'Gratis',
    stripe_checkout: 'Stripe',
    admin_grant: 'Concedido por admin',
  },
  statusLabels: {
    trialing: 'Prueba',
    active: 'Activo',
    past_due: 'Pago pendiente',
    canceled: 'Cancelado',
    expired: 'Expirado',
  },
  filters: {
    search: 'Buscar usuario o ítem…',
    dateFrom: 'Desde',
    dateTo: 'Hasta',
    clear: 'Limpiar filtros',
  },
}

export const catalogEs = {
  premium: {
    badge: 'Catálogo premium',
    viewAll: 'Ver catálogo completo',
    productsTeaser: 'Plataformas SaaS seleccionadas por calidad, gobierno y escala.',
    coursesTeaser: 'Rutas con expertos, laboratorios, plantillas y certificados.',
  },
  typeProduct: 'Producto SaaS',
  typeCourse: 'Curso',
  typeBook: 'Libro',
  typeResource: 'Recurso descargable',
  groupLabels: {
    product: 'Productos',
    course: 'Cursos',
    book: 'Libros',
    resource: 'Recursos',
  },
  viewDetail: 'Ver detalle',
  preview: 'Vista previa',
  addFavorite: 'Añadir a favoritos',
  removeFavorite: 'Quitar de favoritos',
  buy: 'Comprar',
  purchased: 'Comprado',
  free: 'Gratis',
  planSavings: {
    title: 'Podrías ahorrar en esto',
    body: 'Este elemento está incluido en un plan — suscribirte puede costar menos que comprarlo (y todo lo demás que incluye el plan) por separado.',
    planValue: 'Valor si se compra por separado',
    planPrice: 'Precio del plan',
    savingsLabel: 'Ahorras',
    perMonth: '/mes',
    itemsIncludedLabel: 'elementos incluidos',
    buyAlone: 'Comprar solo este elemento',
    seePlan: 'Ver este plan',
  },
  series: {
    title: 'Serie',
    progress: '{{owned}}/{{total}} adquiridos',
    current: 'Estás aquí',
    recommendedNext: 'Te recomendamos seguir con',
    complete: '¡Ya tienes la serie completa!',
  },
  loadError: 'No se pudo cargar el catálogo. Inténtalo más tarde.',
  empty: 'Aún no hay elementos en este catálogo.',
  emptyHint: 'Añadimos productos, cursos, libros y recursos con frecuencia — vuelve pronto.',
  emptyFiltered: 'Ningún elemento coincide con estos filtros.',
  emptyFilteredHint: 'Prueba con otro término de búsqueda o quita las etiquetas seleccionadas.',
  emptyByType: {
    product: {
      title: 'Próximamente: productos ejecutables',
      description: 'Estamos preparando los primeros productos ejecutables. Mientras tanto, explora las skills y plantillas disponibles.',
      cta: 'Explorar recursos',
    },
    course: {
      title: 'Próximamente: rutas formativas',
      description: 'Estamos preparando los primeros cursos. Mientras tanto, explora las skills y plantillas disponibles.',
      cta: 'Explorar recursos',
    },
    book: {
      title: 'Próximamente: libros y guías',
      description: 'Estamos preparando los primeros libros y guías técnicas. Mientras tanto, explora las skills y plantillas disponibles.',
      cta: 'Explorar recursos',
    },
    resource: {
      title: 'Próximamente: nuevos recursos',
      description: 'Estamos preparando nuevos recursos. Mientras tanto, explora los productos y cursos disponibles.',
      cta: 'Explorar productos',
    },
  },
  searchPlaceholder: 'Buscar por título o categoría…',
  benefits: 'Beneficios',
  requirements: 'Requisitos',
  included: 'Contenido incluido',
  tags: {
    filterLabel: 'Etiquetas',
    all: 'Todas',
    clear: 'Limpiar',
  },
  author: 'Autor',
  level: 'Nivel',
  duration: 'Duración',
  category: 'Categoría',
  backToCatalog: 'Volver al catálogo',
  openPreview: 'Abrir vista previa',
  requestAccess: 'Solicitar acceso',
  requestDemo: 'Solicitar demo',
  requestAccessTitle: 'Solicitud de acceso',
  requestDemoTitle: 'Solicitud de demo',
  purchaseSuccess: 'Añadido a tu biblioteca.',
  resource: {
    viewContent: 'Abrir recurso',
    viewPreview: 'Ver muestra',
    download: 'Descargar',
    modalTitle: 'Contenido',
    loadError: 'No se pudo cargar el contenido del archivo.',
    downloadError: 'No se pudo descargar el archivo.',
    truncated: 'El archivo es demasiado grande para mostrarlo completo aquí — descárgalo para ver el contenido completo.',
    empty: 'Este archivo está vacío.',
    copy: 'Copiar',
    copied: 'Copiado',
    renderError: 'No se pudo renderizar este archivo. Prueba a descargarlo.',
    tooLarge: 'Este archivo es demasiado grande para previsualizarlo en el navegador — usa el botón de descarga.',
    xlsxTruncatedRows: 'Se muestran solo las primeras filas de esta hoja — descarga el archivo para verlo completo.',
    close: 'Cerrar',
    loadErrorHint: 'Aun así puedes obtener el archivo directamente.',
    emptyHint: 'Todavía no hay nada que previsualizar para este elemento.',
    kindMarkdown: 'Guía (Markdown)',
    kindDocx: 'Documento Word',
    kindXlsx: 'Hoja de cálculo Excel',
    kindCode: 'Código / script',
    kindPdf: 'PDF',
    codeTruncatedLines: 'Se muestran solo las primeras líneas de este archivo — descárgalo para ver el resto.',
    previewBadge: 'Vista previa',
    previewHint: 'Estás viendo una muestra. Compra este producto para desbloquear el libro completo.',
    maximize: 'Maximizar',
    restore: 'Restaurar',
    downloadFormat: {
      pdf: 'PDF',
      epub: 'EPUB',
      kindle: 'Kindle',
      zip: 'Todos los formatos',
    },
    buyPrinted: 'Comprar impreso',
    buyPrintedComingSoon: 'Próximamente',
    audiobook: 'Audiolibro',
    chapters: 'Capítulos',
    noChapters: 'Este libro todavía no tiene capítulos de audiolibro alojados en la plataforma.',
    chapterLoadError: 'No se pudo cargar este capítulo.',
    previousChapter: 'Capítulo anterior',
    nextChapter: 'Capítulo siguiente',
    formatUnavailable: 'Esta edición todavía no se ha subido.',
    viewer: {
      toc: 'Índice',
      searchPlaceholder: 'Buscar en este documento…',
      clearSearch: 'Limpiar búsqueda',
      noMatches: '0/0',
      prevMatch: 'Coincidencia anterior',
      nextMatch: 'Siguiente coincidencia',
      copySection: 'Copiar esta sección',
      sectionCopied: 'Sección copiada',
      copySheet: 'Copiar hoja',
      sheetCopied: 'Hoja copiada',
      searchRowsPlaceholder: 'Filtrar filas…',
      noRowsMatch: 'Ninguna fila coincide con tu búsqueda.',
      resumedTitle: 'Retomaste donde lo dejaste',
    },
  },
  description: {
    showMore: 'Ver más',
    showLess: 'Ver menos',
  },
  downloadInfo: {
    title: 'Qué incluye',
    files: 'archivos',
  },
  gettingStarted: {
    title: 'Cómo empezar',
  },
  version: {
    title: 'Versión y compatibilidad',
    current: 'Versión actual',
    newVersionAvailable: 'Hay una versión nueva disponible',
    updatedAt: 'Última actualización',
    acquiredAt: 'Fecha de adquisición',
    compatibility: 'Compatible con',
    changelog: 'Historial de cambios',
  },
  license: {
    title: 'Licencia de uso',
    scope: 'Uso permitido',
    scopeUnspecified: 'No especificado — contáctanos si necesitas aclararlo antes de comprar.',
    scopeOptions: {
      individual: 'Uso individual',
      company: 'Uso dentro de una empresa',
    },
    redistribution: 'Redistribución',
    redistributionUnspecified: 'No especificado — contáctanos si necesitas aclararlo antes de comprar.',
    redistributionOptions: {
      prohibited: 'Prohibida',
      allowed: 'Permitida',
      allowed_with_attribution: 'Permitida con atribución',
    },
    updatesIncluded: 'Las futuras actualizaciones están incluidas',
    updatesNotIncluded: 'Las futuras actualizaciones no están incluidas',
    supportIncluded: 'El soporte está incluido',
    supportNotIncluded: 'El soporte no está incluido',
    refundPolicy: 'Política de reembolso',
    refundPolicyDefault: 'Cubierto por nuestra política estándar de reembolso de contenido digital — consulta los Términos de Servicio.',
  },
  levels: {
    beginner: 'Principiante',
    intermediate: 'Intermedio',
    advanced: 'Avanzado',
  },
  rating: {
    upvote: 'Me ayudó',
    downvote: 'No me ayudó',
    removeMyRating: 'Quitar mi valoración',
    tagsHint: '¿Qué destacarías? (hasta 3)',
    votePrompt: 'Elige 👍 o 👎 para guardar tus etiquetas',
    sortTopRated: 'Mejor valorados',
    sortNewest: 'Más recientes',
    infoTooltip:
      '👍 Me ayudó: este elemento fue útil, está bien hecho o mereció la pena. 👎 No me ayudó: no cumplió tus expectativas o no te resultó útil. Tu voto se muestra como un contador público y ayuda a otros usuarios y a la clasificación del catálogo.',
    tags: {
      saves_time: 'Ahorra tiempo',
      high_quality: 'Alta calidad',
      easy_to_apply: 'Fácil de aplicar',
      great_support: 'Buen soporte',
      worth_price: 'Vale su precio',
      too_technical: 'Muy técnico',
      outdated: 'Desactualizado',
      not_worth_price: 'No vale su precio',
    },
  },
  review: {
    title: 'Reseñas',
    writeTitle: 'Deja tu reseña',
    ratingLabel: 'Tu valoración',
    commentPlaceholder: 'Cuenta tu experiencia con este ítem (opcional)',
    submit: 'Enviar reseña',
    update: 'Actualizar reseña',
    remove: 'Eliminar mi reseña',
    purchaseRequired: 'Compra este ítem para poder dejar una reseña.',
    empty: 'Todavía no hay reseñas — sé el primero en escribir una.',
    average: '{{rating}} de 5 ({{count}} reseñas)',
    averageOne: '{{rating}} de 5 (1 reseña)',
    count: '({{count}} reseñas)',
    countOne: '(1 reseña)',
    loadMore: 'Cargar más reseñas',
  },
  share: {
    button: 'Compartir',
    copied: 'Enlace copiado al portapapeles',
    error: 'No se pudo compartir este ítem.',
  },
  pages: {
    products: { title: 'Catálogo de productos', subtitle: 'Herramientas SaaS para tu flujo de trabajo.' },
    courses: { title: 'Catálogo de cursos', subtitle: 'Rutas formativas para tu equipo.' },
    books: { title: 'Catálogo de libros', subtitle: 'Libros técnicos y guías de referencia.' },
    resources: { title: 'Catálogo de recursos', subtitle: 'Plantillas y kits descargables.' },
    favorites: { title: 'Favoritos', subtitle: 'Elementos guardados para más tarde.' },
    purchases: { title: 'Mis compras', subtitle: 'Todo lo que has adquirido.' },
    myProducts: { title: 'Mis productos', subtitle: 'Productos en tu biblioteca.' },
    myCourses: { title: 'Mis cursos', subtitle: 'Cursos en tu biblioteca.' },
    myBooks: { title: 'Mis libros', subtitle: 'Libros en tu biblioteca.' },
    myResources: { title: 'Mis recursos', subtitle: 'Recursos descargables que posees.' },
    myLibrary: {
      title: 'Mi biblioteca',
      subtitle: 'Todo lo que tienes, en un solo sitio — cambia de pestaña para filtrar por tipo.',
      tabs: { all: 'Todos' },
    },
  },
  myPurchases: {
    moreInfo: 'Más información',
    hide: 'Ocultar',
    access: 'Acceder',
    download: 'Descargar',
    requestSupport: 'Solicitar soporte',
    includedItems: 'Elementos incluidos',
    purchasedAt: 'Fecha de compra',
    amountPaid: 'Precio pagado',
    discount: 'Descuento aplicado',
    none: 'Ninguno',
    method: 'Método de adquisición',
    invoice: 'Factura',
    viewInvoice: 'Ver factura',
    notAvailable: 'No disponible',
    paymentStatus: 'Estado del pago',
    version: 'Versión adquirida',
    standardVersion: 'Estándar',
    organizationVia: 'Vía organización',
    loadError: 'No se pudieron cargar tus compras.',
    empty: 'Todavía no has comprado nada.',
    emptyHint: 'Todo lo que compres aparecerá aquí con el detalle completo: fecha, precio, factura y más.',
    detailError: 'No se pudo cargar el detalle de esta compra.',
    supportModalTitle: 'Solicitar soporte',
    supportPlaceholder: 'Describe el problema que tienes con este elemento...',
    supportSent: 'Tu solicitud ha sido enviada — te responderemos pronto.',
    supportError: 'No se pudo enviar tu solicitud. Inténtalo de nuevo.',
    send: 'Enviar',
    sending: 'Enviando...',
    cancel: 'Cancelar',
    source: {
      free: 'Reclamación gratuita',
      stripe_checkout: 'Pago con tarjeta (Stripe)',
      plan_entitlement: 'Incluido en el plan',
      admin_grant: 'Concedido por un administrador',
    },
    paymentStatusLabels: {
      paid: 'Pagado',
      unpaid: 'Sin pagar',
      no_payment_required: 'No requiere pago',
      not_applicable: 'No aplica',
      unknown: 'Desconocido',
    },
  },
}

export const independentDashboardEn = {
  title: 'Your workspace',
  heroBadge: 'Independent account',
  subtitle: 'Discover premium products and courses — no organization management required.',
  explore: 'Explore',
  cards: {
    products: 'Discover SaaS products',
    courses: 'Training and certifications',
    books: 'Technical reading',
    resources: 'Templates and downloads',
    favorites: 'Saved items',
    purchases: 'Purchase history',
    myLibrary: 'My library',
    myCourses: 'Enrolled courses',
    myBooks: 'Your digital shelf',
    requests: 'Access requests',
    profile: 'Personal settings',
  },
  progress: {
    badge: 'Your activity',
    title: 'Your progress',
    subtitle: 'A real snapshot of what you have collected so far — no guesswork.',
    emptyTitle: 'Nothing purchased yet',
    emptyBody: 'Once you buy your first product, course, book or resource, your progress will show up here.',
    itemsLabel: 'items',
    totalSpent: 'Total invested',
    itemsOwned: 'Items owned',
    favoritesSaved: 'Saved favorites',
    planSavings: 'Saved with your plan',
  },
  recommended: {
    badge: 'Keep exploring',
  },
  continueLearning: {
    badge: 'Continue where you left off',
    cta: 'Continue',
  },
  news: {
    badge: "What's new",
  },
  categoryCharts: {
    badge: 'Breakdown',
    title: 'Spending and saves by category',
    subtitle: 'Where your budget and your bookmarks are going, category by category.',
    spendTitle: 'Spend by category',
    savedTitle: 'Saved by category',
    spendEmpty: 'No purchases yet',
    savedEmpty: 'No favorites yet',
  },
  upsell: {
    title: 'Get more with a plan',
    body: 'Subscribe to a plan to unlock a curated set of products, courses, books and resources included at no extra cost.',
    cta: 'See plans',
  },
}

export const independentDashboardEs = {
  title: 'Tu espacio',
  heroBadge: 'Cuenta independiente',
  subtitle: 'Descubre productos y cursos premium — sin gestión organizacional.',
  explore: 'Explorar',
  cards: {
    products: 'Descubre productos SaaS',
    courses: 'Formación y certificaciones',
    books: 'Lectura técnica',
    resources: 'Plantillas y descargas',
    favorites: 'Elementos guardados',
    purchases: 'Historial de compras',
    myLibrary: 'Mi biblioteca',
    myCourses: 'Cursos adquiridos',
    myBooks: 'Tu estantería digital',
    requests: 'Solicitudes de acceso',
    profile: 'Ajustes personales',
  },
  progress: {
    badge: 'Tu actividad',
    title: 'Tu progreso',
    subtitle: 'Una foto real de lo que has ido reuniendo hasta ahora — sin adivinar nada.',
    emptyTitle: 'Aún no has comprado nada',
    emptyBody: 'En cuanto compres tu primer producto, curso, libro o recurso, tu progreso aparecerá aquí.',
    itemsLabel: 'elementos',
    totalSpent: 'Total invertido',
    itemsOwned: 'Elementos en tu poder',
    favoritesSaved: 'Favoritos guardados',
    planSavings: 'Ahorrado con tu plan',
  },
  recommended: {
    badge: 'Seguí explorando',
  },
  continueLearning: {
    badge: 'Continúa donde lo dejaste',
    cta: 'Continuar',
  },
  news: {
    badge: 'Novedades',
  },
  categoryCharts: {
    badge: 'Desglose',
    title: 'Gasto y guardados por categoría',
    subtitle: 'A dónde va tu presupuesto y qué estás guardando, categoría por categoría.',
    spendTitle: 'Gasto por categoría',
    savedTitle: 'Guardados por categoría',
    spendEmpty: 'Aún no hay compras',
    savedEmpty: 'Aún no hay favoritos',
  },
  upsell: {
    title: 'Consigue más con un plan',
    body: 'Suscríbete a un plan para desbloquear una selección de productos, cursos, libros y recursos incluidos sin coste extra.',
    cta: 'Ver planes',
  },
}

export const profilePageEn = {
  title: 'Personal profile',
  subtitle: 'Update your photo and account details.',
  accountSection: 'Account details',
  securitySection: 'Security',
  phone: 'Phone number',
  phonePlaceholder: '+34 600 111 222',
  phoneHint: 'International format (E.164). Required for SMS verification and two-factor authentication.',
  phoneVerified: 'Verified',
  phoneNotVerified: 'Not verified',
  verifyPhoneSoon: 'Verify via SMS — coming soon',
  twoFactor: 'Two-factor authentication (2FA)',
  twoFactorEnabled: 'Enabled',
  twoFactorDisabled: 'Not enabled',
  twoFactorDescription: 'Add a second step at login using an authenticator app (Google Authenticator, Authy, 1Password...).',
  twoFactorEnabledHint: 'Your account is protected with an authenticator app code at every login.',
  twoFactorEnableCta: 'Enable 2FA',
  twoFactorDisableCta: 'Disable 2FA',
  twoFactorLoading: 'Loading…',
  twoFactorScanHint: 'Scan this QR code with your authenticator app, then enter the 6-digit code it shows.',
  twoFactorQrAlt: '2FA setup QR code',
  twoFactorManualHint: "Can't scan it? Enter this code manually:",
  twoFactorCodeLabel: '6-digit code',
  twoFactorConfirmCta: 'Confirm and enable',
  twoFactorCancel: 'Cancel',
  twoFactorDisableHint: 'Enter your account password to turn off two-factor authentication.',
  twoFactorPasswordPlaceholder: 'Your password',
  twoFactorSetupError: 'Could not start 2FA setup. Try again.',
  twoFactorConfirmError: 'Invalid code. Check your authenticator app and try again.',
  twoFactorDisableError: 'Incorrect password.',
  email: 'Email',
  firstName: 'First name',
  lastName: 'Last name',
  displayName: 'Display name',
  name: 'Name',
  role: 'Primary role',
  photo: 'Profile photo',
  photoHint: 'JPEG, PNG or WebP up to 2 MB. Saved in the database.',
  uploadPhoto: 'Upload photo',
  photoSaved: 'Photo saved.',
  uploadError: 'Could not save the photo. Try again.',
  save: 'Save changes',
  saved: 'Profile updated.',
  saveError: 'Could not save your profile. Check the phone format (e.g. +34600111222).',
  independentNote: 'You are an independent user without organization management features.',
  adminNote: 'Platform administrator account. Catalog and user management are in the admin menu.',
  tenureLabel: 'On the platform for',
  billing: {
    title: 'Billing',
    subtitle: 'Manage your plan, payment method and invoices.',
    currentPlan: 'Current plan',
    freePlan: 'Free',
    manageButton: 'Manage billing',
    viewPlans: 'See plans',
    opening: 'Opening…',
    error: 'Could not open the billing portal. Try again.',
    noAccount: "You don't have a billing account yet — subscribe to a paid plan first to manage payments and invoices here.",
    startedOn: 'Active since',
    nextCharge: 'Next charge',
    endsOnPrefix: 'Your plan stays active until',
    endsOnSuffix: 'It will not renew after that date.',
    cancelScheduledBadge: 'Cancellation scheduled',
    cancelButton: 'Cancel subscription',
    cancelling: 'Cancelling…',
    cancelError: 'Could not cancel your subscription. Please try again.',
    resumeButton: 'Resume subscription',
    resuming: 'Resuming…',
    resumeError: 'Could not resume your subscription. Please try again.',
    cancelModalTitle: 'Cancel your subscription?',
    cancelModalBodyPrefix: "You'll keep full access until",
    cancelModalBodySuffix: 'After that date your plan will not renew and plan-only resources will no longer be available.',
    cancelModalKeep: 'Keep my plan',
    cancelModalConfirm: 'Yes, cancel',
    status: {
      trialing: 'Trial',
      active: 'Active',
      past_due: 'Payment overdue',
      canceled: 'Canceled',
      expired: 'Expired',
    },
    invoices: {
      title: 'Invoice history',
      subtitle: 'Every receipt for your ASE subscription, in one place.',
      empty: 'No invoices yet.',
      loadError: 'Could not load your invoice history.',
      colDate: 'Date',
      colPlan: 'Plan',
      colAmount: 'Amount',
      colStatus: 'Status',
      colActions: 'Receipt',
      viewPdf: 'View PDF',
      viewOnline: 'View online',
      status: {
        paid: 'Paid',
        open: 'Pending',
        void: 'Voided',
        uncollectible: 'Uncollectible',
        draft: 'Draft',
      },
    },
  },
  newsletter: {
    title: 'Weekly newsletter',
    subtitle: 'A short Friday digest with new members, new content and platform news. You can unsubscribe anytime.',
    toggleLabel: 'Receive the weekly newsletter',
    saved: 'Preference saved.',
    error: 'Could not update your preference. Try again.',
  },
}

export const profilePageEs = {
  title: 'Perfil personal',
  subtitle: 'Actualiza tu foto y datos de cuenta.',
  accountSection: 'Datos de la cuenta',
  securitySection: 'Seguridad',
  phone: 'Teléfono',
  phonePlaceholder: '+34 600 111 222',
  phoneHint: 'Formato internacional (E.164). Necesario para verificación por SMS y doble factor.',
  phoneVerified: 'Verificado',
  phoneNotVerified: 'Sin verificar',
  verifyPhoneSoon: 'Verificar por SMS — próximamente',
  twoFactor: 'Autenticación en dos pasos (2FA)',
  twoFactorEnabled: 'Activado',
  twoFactorDisabled: 'Desactivado',
  twoFactorDescription: 'Añade un segundo paso al iniciar sesión usando una app autenticadora (Google Authenticator, Authy, 1Password...).',
  twoFactorEnabledHint: 'Tu cuenta está protegida con un código de app autenticadora en cada inicio de sesión.',
  twoFactorEnableCta: 'Activar 2FA',
  twoFactorDisableCta: 'Desactivar 2FA',
  twoFactorLoading: 'Cargando…',
  twoFactorScanHint: 'Escanea este código QR con tu app autenticadora e introduce el código de 6 dígitos que te muestre.',
  twoFactorQrAlt: 'Código QR para activar 2FA',
  twoFactorManualHint: '¿No puedes escanearlo? Introduce este código manualmente:',
  twoFactorCodeLabel: 'Código de 6 dígitos',
  twoFactorConfirmCta: 'Confirmar y activar',
  twoFactorCancel: 'Cancelar',
  twoFactorDisableHint: 'Introduce la contraseña de tu cuenta para desactivar la autenticación en dos pasos.',
  twoFactorPasswordPlaceholder: 'Tu contraseña',
  twoFactorSetupError: 'No se pudo iniciar la configuración del 2FA. Inténtalo de nuevo.',
  twoFactorConfirmError: 'Código no válido. Revisa tu app autenticadora e inténtalo de nuevo.',
  twoFactorDisableError: 'Contraseña incorrecta.',
  email: 'Correo',
  firstName: 'Nombre',
  lastName: 'Apellidos',
  displayName: 'Nombre visible',
  name: 'Nombre',
  role: 'Rol principal',
  photo: 'Foto de perfil',
  photoHint: 'JPEG, PNG o WebP hasta 2 MB. Se guarda en la base de datos.',
  uploadPhoto: 'Subir foto',
  photoSaved: 'Foto guardada.',
  uploadError: 'No se pudo guardar la foto. Inténtalo de nuevo.',
  save: 'Guardar cambios',
  saved: 'Perfil actualizado.',
  saveError: 'No se pudo guardar el perfil. Revisa el formato del teléfono (ej. +34600111222).',
  independentNote: 'Eres un usuario independiente sin funciones de gestión organizacional.',
  adminNote: 'Cuenta de administrador de plataforma. Catálogo y usuarios están en el menú de administración.',
  tenureLabel: 'Antigüedad en la plataforma',
  billing: {
    title: 'Facturación',
    subtitle: 'Gestiona tu plan, método de pago y facturas.',
    currentPlan: 'Plan actual',
    freePlan: 'Gratuito',
    manageButton: 'Gestionar facturación',
    viewPlans: 'Ver planes',
    opening: 'Abriendo…',
    error: 'No se pudo abrir el portal de facturación. Inténtalo de nuevo.',
    noAccount: 'Todavía no tienes una cuenta de facturación — primero debes suscribirte a un plan de pago para poder gestionar aquí tus pagos y facturas.',
    startedOn: 'Activo desde',
    nextCharge: 'Próximo cobro',
    endsOnPrefix: 'Tu plan seguirá activo hasta el',
    endsOnSuffix: 'No se renovará a partir de esa fecha.',
    cancelScheduledBadge: 'Cancelación programada',
    cancelButton: 'Cancelar suscripción',
    cancelling: 'Cancelando…',
    cancelError: 'No se pudo cancelar tu suscripción. Inténtalo de nuevo.',
    resumeButton: 'Reactivar suscripción',
    resuming: 'Reactivando…',
    resumeError: 'No se pudo reactivar tu suscripción. Inténtalo de nuevo.',
    cancelModalTitle: '¿Cancelar tu suscripción?',
    cancelModalBodyPrefix: 'Mantendrás el acceso completo hasta el',
    cancelModalBodySuffix: 'A partir de esa fecha tu plan no se renovará y perderás acceso a los recursos exclusivos del plan.',
    cancelModalKeep: 'Mantener mi plan',
    cancelModalConfirm: 'Sí, cancelar',
    status: {
      trialing: 'Prueba',
      active: 'Activo',
      past_due: 'Pago pendiente',
      canceled: 'Cancelado',
      expired: 'Expirado',
    },
    invoices: {
      title: 'Historial de facturas',
      subtitle: 'Todos los recibos de tu suscripción ASE, en un solo lugar.',
      empty: 'Todavía no hay facturas.',
      loadError: 'No se pudo cargar tu historial de facturas.',
      colDate: 'Fecha',
      colPlan: 'Plan',
      colAmount: 'Importe',
      colStatus: 'Estado',
      colActions: 'Recibo',
      viewPdf: 'Ver PDF',
      viewOnline: 'Ver online',
      status: {
        paid: 'Pagada',
        open: 'Pendiente',
        void: 'Anulada',
        uncollectible: 'Incobrable',
        draft: 'Borrador',
      },
    },
  },
  newsletter: {
    title: 'Newsletter semanal',
    subtitle: 'Un resumen breve cada viernes con nuevos miembros, contenido nuevo y noticias de la plataforma. Puedes darte de baja cuando quieras.',
    toggleLabel: 'Recibir la newsletter semanal',
    saved: 'Preferencia guardada.',
    error: 'No se pudo actualizar tu preferencia. Inténtalo de nuevo.',
  },
}

export const servicesAdminEn = {
  badge: 'Platform services',
  title: 'Services management',
  subtitle: 'Create, edit and deactivate the services, frameworks and offerings shown across the platform.',
  common: { na: 'N/A' },
  stats: {
    total: 'Total services',
    active: 'Active',
    featured: 'Featured',
    categories: 'Categories in use',
  },
  filters: {
    search: 'Search',
    searchPlaceholder: 'Search by name, code or slug…',
    category: 'Category',
    status: 'Status',
    all: 'All',
  },
  list: {
    columns: {
      service: 'Service',
      category: 'Category',
      type: 'Type',
      status: 'Status',
      actions: 'Actions',
    },
  },
  empty: { title: 'No services yet', subtitle: 'Create your first service using the form on the right.' },
  errors: { loadTitle: 'Could not load services', loadSubtitle: 'Try again in a moment.' },
  badges: { active: 'Active', inactive: 'Inactive', featured: 'Featured' },
  actions: {
    edit: 'Edit',
    deactivate: 'Deactivate',
    addFeature: 'Add',
    remove: 'Remove',
  },
  create: {
    title: 'Create service',
    subtitle: 'Optimized form — only name, code and slug are required.',
    button: 'Create service',
    creating: 'Creating…',
    error: 'Could not create the service. Check the fields and try again.',
  },
  edit: {
    title: 'Edit service',
    save: 'Save changes',
    saving: 'Saving…',
    error: 'Could not save changes.',
  },
  delete: {
    title: 'Deactivate service',
    body: 'Deactivate "{{name}}"? It will no longer be shown publicly, but its history is kept.',
    note: 'You can reactivate it later by editing it and turning "Active" back on.',
    confirm: 'Deactivate',
    deleting: 'Deactivating…',
    error: 'Could not deactivate the service.',
  },
  fields: {
    name: 'Name',
    code: 'Code',
    slug: 'Slug',
    shortDescription: 'Short description',
    description: 'Full description',
    category: 'Category',
    type: 'Type',
    priceType: 'Price type',
    price: 'Price (optional)',
    displayOrder: 'Display order',
    icon: 'Icon (identifier)',
    isActive: 'Active',
    isFeatured: 'Featured',
    features: 'Features',
  },
  placeholders: {
    name: 'e.g. QA Automation Sprint',
    code: 'e.g. qa-automation-sprint',
    slug: 'e.g. qa-automation-sprint',
    shortDescription: 'One line summary shown in listings',
    description: 'Full description shown on the service detail page',
    icon: 'e.g. rocket',
    featureInput: 'Add a feature and press Add',
  },
  categories: {
    platform_engineering: 'Platform engineering',
    qa_automation: 'QA automation',
    training: 'Training',
    digital_products: 'Digital products',
    consulting: 'Consulting',
    ai_automation: 'AI automation',
    frameworks: 'Frameworks',
  },
  kinds: {
    service: 'Service',
    product: 'Product',
    framework: 'Framework',
    course: 'Course',
    book: 'Book',
  },
  priceTypes: {
    free: 'Free',
    fixed: 'Fixed price',
    subscription: 'Subscription',
    custom: 'Custom / on request',
  },
}

export const servicesAdminEs = {
  badge: 'Servicios de la plataforma',
  title: 'Gestión de servicios',
  subtitle: 'Crea, edita y desactiva los servicios, frameworks y ofertas que se muestran en toda la plataforma.',
  common: { na: 'N/D' },
  stats: {
    total: 'Servicios totales',
    active: 'Activos',
    featured: 'Destacados',
    categories: 'Categorías en uso',
  },
  filters: {
    search: 'Buscar',
    searchPlaceholder: 'Buscar por nombre, código o slug…',
    category: 'Categoría',
    status: 'Estado',
    all: 'Todos',
  },
  list: {
    columns: {
      service: 'Servicio',
      category: 'Categoría',
      type: 'Tipo',
      status: 'Estado',
      actions: 'Acciones',
    },
  },
  empty: { title: 'Todavía no hay servicios', subtitle: 'Crea tu primer servicio con el formulario de la derecha.' },
  errors: { loadTitle: 'No se pudieron cargar los servicios', loadSubtitle: 'Inténtalo de nuevo en un momento.' },
  badges: { active: 'Activo', inactive: 'Inactivo', featured: 'Destacado' },
  actions: {
    edit: 'Editar',
    deactivate: 'Desactivar',
    addFeature: 'Añadir',
    remove: 'Quitar',
  },
  create: {
    title: 'Crear servicio',
    subtitle: 'Formulario optimizado — solo nombre, código y slug son obligatorios.',
    button: 'Crear servicio',
    creating: 'Creando…',
    error: 'No se pudo crear el servicio. Revisa los campos e inténtalo de nuevo.',
  },
  edit: {
    title: 'Editar servicio',
    save: 'Guardar cambios',
    saving: 'Guardando…',
    error: 'No se pudieron guardar los cambios.',
  },
  delete: {
    title: 'Desactivar servicio',
    body: '¿Desactivar "{{name}}"? Dejará de mostrarse públicamente, pero se conserva su historial.',
    note: 'Puedes reactivarlo más tarde editándolo y volviendo a activar "Activo".',
    confirm: 'Desactivar',
    deleting: 'Desactivando…',
    error: 'No se pudo desactivar el servicio.',
  },
  fields: {
    name: 'Nombre',
    code: 'Código',
    slug: 'Slug',
    shortDescription: 'Descripción corta',
    description: 'Descripción completa',
    category: 'Categoría',
    type: 'Tipo',
    priceType: 'Tipo de precio',
    price: 'Precio (opcional)',
    displayOrder: 'Orden de visualización',
    icon: 'Icono (identificador)',
    isActive: 'Activo',
    isFeatured: 'Destacado',
    features: 'Características',
  },
  placeholders: {
    name: 'ej. Sprint de automatización QA',
    code: 'ej. qa-automation-sprint',
    slug: 'ej. qa-automation-sprint',
    shortDescription: 'Resumen de una línea mostrado en los listados',
    description: 'Descripción completa mostrada en el detalle del servicio',
    icon: 'ej. rocket',
    featureInput: 'Añade una característica y pulsa Añadir',
  },
  categories: {
    platform_engineering: 'Ingeniería de plataforma',
    qa_automation: 'Automatización QA',
    training: 'Formación',
    digital_products: 'Productos digitales',
    consulting: 'Consultoría',
    ai_automation: 'Automatización con IA',
    frameworks: 'Frameworks',
  },
  kinds: {
    service: 'Servicio',
    product: 'Producto',
    framework: 'Framework',
    course: 'Curso',
    book: 'Libro',
  },
  priceTypes: {
    free: 'Gratis',
    fixed: 'Precio fijo',
    subscription: 'Suscripción',
    custom: 'Personalizado / a consultar',
  },
}
